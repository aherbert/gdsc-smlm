 ...
 * <p>
 * This class is based on the original algorithm description. It can have large errors when the unbiased exponent is
 * zero.
 *
 * @see <a href=
...
public class ICSIFastLog extends FastLog
{
	 ...
	 * The table has had the float bias (-127) pre-subtracted.
	 */
	private final float[] data;
	/**
	 * The table of the log2 value of binary number 1.0000... to 1.1111...., depending on the precision.
	 * The table has had the double bias (-1023) pre-subtracted.
	 */
	 ...
	 * <p>
	 * If the table is not initialised then a call to a log function with the particular data type will throw a null
	 * pointer exception.
	 *
	 * @param dataType
	 *            the data type
	 ...
	 *             if n is not in the range 0-23, or no datatype is specified
	 */
	 ...
	 * hold the pre-calculated log values for base 2
	...
	private ICSIFastLog(int n, DataType dataType)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			//assert gdsc.core.utils.FloatEquality.almostEqualRelativeOrAbsolute(log2, fastLog2(value), 1e-6f,
			//		1e-16f) : String.format("[%d] data[i](%g)  %g != %g  %g", i, value, log2, fastLog2(value),
			//				gdsc.core.utils.FloatEquality.relativeError(log2, fastLog2(value)));
		}
	}
