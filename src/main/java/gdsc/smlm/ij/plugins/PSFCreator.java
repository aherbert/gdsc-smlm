...
public class PSFCreator implements PlugInFilter
{
	...
	private void runUsingFitting()
	{
		...
		ArrayList<double[]> centres = new ArrayList<>(spots.length);
		int iterations = 1;
	...
	{
		List<Future<?>> futures = new TurboList<>(centres.length);
	...
	private float[][] align2D(ExtractedPSF combined, final ExtractedPSF[] psfs)
	{
		...
		int n = psfs.length * 3;
		List<Future<?>> futures = new TurboList<>(n);
	...
	private float[][] align3D(ExtractedPSF combined, final ExtractedPSF[] psfs)
	{
		...
		List<Future<?>> futures = new TurboList<>(psfs.length);
