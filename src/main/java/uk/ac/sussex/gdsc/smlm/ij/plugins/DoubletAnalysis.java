...
public class DoubletAnalysis implements PlugIn, ItemListener
{
                    ...
                    if (result.good1)
                    {
                                ...
                                height, cx, cy, fitParams[Gaussian2DFunction.X_SD], fitParams[Gaussian2DFunction.Y_SD]))
                        {
                            ...
                            if (r2 != 0)
                            {
                                ...
                                result.aic1 = MathUtils.getAkaikeInformationCriterionFromResiduals(result.sumOfSquares1,
                                        length, result.fitResult1.getNumberOfFittedParameters());
                                result.aic2 = MathUtils.getAkaikeInformationCriterionFromResiduals(result.sumOfSquares2,
                                        length, result.fitResult2.getNumberOfFittedParameters());
                                result.bic1 = MathUtils.getBayesianInformationCriterionFromResiduals(result.sumOfSquares1,
                                        length, result.fitResult1.getNumberOfFittedParameters());
                                result.bic2 = MathUtils.getBayesianInformationCriterionFromResiduals(result.sumOfSquares2,
                                        length, result.fitResult2.getNumberOfFittedParameters());
            ...
            if (matching == 0)
            {
                ...
                for (final DoubletResult result : frameResults)
                    if (result.good1)
                    {
                        ...
                        final double x = result.fitResult1.getParameters()[Gaussian2DFunction.X_POSITION] +
                                regionBounds.x;
                        final double y = result.fitResult1.getParameters()[Gaussian2DFunction.Y_POSITION] +
                                regionBounds.y;
                ...
                {
                    final ResultCoordinate coord = (ResultCoordinate) pair.getPoint2();
                    coord.result.addTP2(getScore(pair.getXyDistanceSquared(), coord, pair.getPoint1()), coord.id);
                }
            }
            else if (matching == 1)
            {
                ...
                OUTER: for (int j = 0; j < frameResults.size(); j++)
                {
                    ...
                    {
                        final Rectangle regionBounds = ie.getBoxRegionBounds(result.spot.x, result.spot.y, fitting);
                        final float x = (float) (result.fitResult1.getParameters()[Gaussian2DFunction.X_POSITION] +
                                regionBounds.x);
                        final float y = (float) (result.fitResult1.getParameters()[Gaussian2DFunction.Y_POSITION] +
                                regionBounds.y);
                ...
                OUTER: for (int j = 0; j < frameResults.size(); j++)
                {
                    ...
                    if (result.good1)
                    {
                        ...
                        if (result.good2)
                        {
                            final float x1 = (float) (result.fitResult2.getParameters()[Gaussian2DFunction.X_POSITION] +
                                    regionBounds.x);
                            final float y1 = (float) (result.fitResult2.getParameters()[Gaussian2DFunction.Y_POSITION] +
                                    regionBounds.y);
                            final float x2 = (float) (result.fitResult2.getParameters()[Gaussian2DFunction.X_POSITION +
                                    Gaussian2DFunction.PARAMETERS_PER_PEAK] + regionBounds.x);
                            final float y2 = (float) (result.fitResult2.getParameters()[Gaussian2DFunction.Y_POSITION +
                                    Gaussian2DFunction.PARAMETERS_PER_PEAK] + regionBounds.y);
    ...
    private boolean showDialog()
    {
        ...
        gd.addMessage(
                String.format("Fits the benchmark image created by CreateData plugin.\nPSF width = %s, adjusted = %s",
                        MathUtils.rounded(simulationParameters.s / simulationParameters.a), MathUtils.rounded(sa)));
    ...
    private void run()
    {
        ...
        final TIntObjectHashMap<ArrayList<Coordinate>> actualCoordinates = ResultsMatchCalculator
    ...
    private void summariseResults(ArrayList<DoubletResult> results, double density, long runTime)
    {
        ...
        sb.append(MathUtils.rounded(getSa() * simulationParameters.a)).append('\t');
        ...
        final StoredDataStatistics[] stats = new StoredDataStatistics[NAMES2.length];
        ...
        for (int c = 0; c < stats.length; c++)
        {
            ...
            sb.append(MathUtils.rounded(stats[c].getMean())).append("+/-")
                    .append(MathUtils.rounded(stats[c].getStandardDeviation())).append(" (").append(stats[c].getN())
                    .append(") ").append(MathUtils.rounded(p.evaluate(values))).append('\t');
    ...
    private void runAnalysis()
    {
        ...
        final ArrayList<DoubletBonus> data = new ArrayList<>(doubletResults.size());
        // True positive and False positives at residuals = 0
        double tp = 0;
        double fp = 0;

        final Logger logger = (analysisLogging) ? ImageJPluginLoggerHelper.getLogger(getClass()) : null;
        ...
        for (final DoubletResult result : doubletResults)
            // Filter the singles that would be accepted
            if (result.good1)
            {
                filterFitConfig.setNoise(result.noise);
                final FitStatus fitStatus0 = filterFitConfig.validatePeak(0, result.fitResult1.getInitialParameters(),
                        ...
                        result.good2)
                {
                    ...
                    final FitStatus fitStatus1 = filterFitConfig2.validatePeak(0,
                            result.fitResult2.getInitialParameters(), result.fitResult2.getParameters(),
                            result.fitResult2.getParameterDeviations());
                    final FitStatus fitStatus2 = filterFitConfig2.validatePeak(1,
                            result.fitResult2.getInitialParameters(), result.fitResult2.getParameters(),
                            result.fitResult2.getParameterDeviations());
                    ...
                    if (accept[0] || accept[1])
                        if (result.gap < minGap)
                        {
                            ...
                            if (logger != null)
                                LoggerUtils.log(logger, Level.INFO, "Reject Doublet (%.2f): Fitted coordinates below min gap (%g<%g)",
    ...
    private void saveTemplate(String summary)
    {
        ...
        final String filename = BenchmarkFilterAnalysis.getFilename("Template_File", templateFilename);
        if (filename != null)
        {
    ...
    private static void getNotes(TemplateSettings.Builder settings, String summary)
    {
    ...
    {
        LoggerUtils.log(logger, Level.INFO, "Reject P%d (%.2f): %s", i, result.getMaxScore(), fitStatus.toString());
    }

    private static boolean selectFit(DoubletResult result)
    {
    ...
    private static double addRange(StringBuilder sb, double[] residuals, double[] jaccard, int maxJaccardIndex,
            double limit)
    {
        double sum = 0;
        ...
        sb.append('\t').append(MathUtils.rounded(sum)).append('\t').append(MathUtils.rounded(lower)).append('\t')
                .append(MathUtils.rounded(upper)).append('\t').append(MathUtils.rounded(upper - lower)).append('\t')
                .append(MathUtils.rounded(weightedMean));
        return weightedMean;
    }

    private static double getArea(double[] residuals, double[] jaccard, int maxJaccardIndex, double window)
    {
        double sum = 0;
    ...
    private boolean showAnalysisDialog()
    {
        ...
        //sb.append("SD1 = ").append(MathUtils.rounded(fitConfig.getInitialPeakStdDev1())).append("\n");
        ...
        if (e.getSource() instanceof Choice)
        {
            ...
            else if (template != null)
            {
                if (template.hasFitEngineSettings())
                {
                    ...
                    final FitEngineConfiguration config2 = new FitEngineConfiguration(template.getFitEngineSettings(),
