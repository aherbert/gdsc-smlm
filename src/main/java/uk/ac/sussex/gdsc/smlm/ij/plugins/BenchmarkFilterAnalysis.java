 ...
 * BenchmarkSpotFit plugin. <p> Filtering is done using e.g. SNR threshold, Precision thresholds,
 * etc. The statistics reported are shown in a table, e.g. precision, Jaccard, F-score.
 */
public class BenchmarkFilterAnalysis implements PlugIn, FitnessFunction<FilterScore>, TrackProgress,
  ...
  private class CustomFractionalAssignment extends PeakFractionalAssignment {
    ...
    public CustomFractionalAssignment(int targetId, int predictedId, double distance, double score,
        double d, PreprocessedPeakResult spot, PeakResult peak) {
   ...
   * parameters array to the lower bounds. Set the input parameters2 array to the upper bounds. <p>
   * If a parameter is not expanded since the increment is Infinity the parameter is disabled. If it
   * was not expanded for any other reason (increment is zero, NaN values, etc) the weakest
  ...
  private MultiPathFitResults[] readResults() {
    ...
    if (update || !equalScoreSettings || lastDuplicateDistance != duplicateDistance
      ...
      // candidates.
      // The ideal combination would correctly fit+pick all the candidate positions that are close
      // to a
      // localisation.
      //
      ...
      // possible
      // actual results and keep a record of the scores. These can then be assessed when the
      // specific
      ...
      // Using a signal-factor ramped scoring function the degree of fitted can be varied from 0 to
      // 1.
      // When using ramped scoring functions the fractional allocation of scores using the above
      // scheme
      // is performed, i.e. candidates are treated as if they both match and unmatch. This results
      // in
      ...
      // TP
      // value but calculating FN = localisations - TP and FP as Positives - TP. This creates a
      // score
      // against the original number of simulated molecules using everything that was passed through
      // the
      // filter (Positives). This score is comparable when a different spot candidate filter has
      // been used
  ...
  private static IdPeakResult[] getCoordinates(TIntObjectHashMap<IdPeakResult[]> coords,
      Integer t) {
  ...
  private void addSimulationData(GenericDialog gd) {
    ...
    final double pSignal = CreateData.getPrecisionN(simulationParameters.a, simulationParameters.s,
        signal, MathUtils.pow2(simulationParameters.noise), simulationParameters.isEMCCD());
    final double pLSE = Gaussian2DPeakResultHelper.getPrecision(simulationParameters.a,
   ...
   * statistics on the success of the filter to an ImageJ table. <p> For each filter set a plot is
   * shown of the score verses the filter value, thus filters should be provided in ascending
   * numerical order otherwise they are sorted.
   *
  ...
  /**
   * Run the optimum filter on a set of labelled peak results using various parameter settings
   * outputting performance statistics on the success of the filter to an ImageJ table. <p> If a new
   * optimum is found the class level static parameters are updated.
   *
  ...
  private void calculateSensitivity() {
    ...
    if (!bestFilter.isEmpty()) {
      ...
      for (final String type : bestFilterOrder) {
        ...
        FractionClassificationResult s =
  ...
  private static ClassificationResult createIntegerResult(FractionClassificationResult r) {
    return new ClassificationResult(r.getNumberOfPositives(), r.getNumberOfNegatives(), 0,
        nActual - r.getNumberOfPositives());
  }
  ...
  /**
   * Save the filter set to a file prompted from the user. <p> If non-interactive then the last
   * filename will be used. This will overwrite existing files if multiple filter sets are used.
   *
  ...
  private ComplexFilterScore score(DirectFilter f, int i, int size, int[] combinations,
      boolean[] enable, int[] uniqueIds1, int uniqueIdCount1) {
  ...
  private class ComplexFilterScore extends SimpleFilterScore {
    ...
    private ComplexFilterScore(FilterScoreResult r, boolean allSameType, char[] atLimit, int index,
        long time, ClassificationResult r2, int size, int[] combinations, boolean[] enable) {
   ...
   */
  private static boolean outsideBorder(PeakResult p, final float border, final float xlimit,
      final float ylimit) {
    // Respect the border used in BenchmarkSpotFilter?
    // Just implement a hard border
   ...
   * @return true, if outside the border (should be ignored)
   */
  private static boolean outsideBorder(BasePreprocessedPeakResult p, final float border,
