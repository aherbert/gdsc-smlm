...
public class ImagePsfModel extends PsfModel {
  ...
  private static double[] calculateCumulativeImage(double[] image) {
    final double[] c = new double[image.length + 1];
  ...
  private void calculateRollingSums(double[] rollingSum) {
    ...
    for (int y = 1, i = maxx; y < maxy; y++) {
      ...
      for (int x = 0; x < maxx; x++, i++) {
        cs += rollingSum[i];
        rollingSum[i] = (rollingSum[i - maxx] + cs);
      }
    }
  }
  ...
  private double safeSum(double[] rollingSum, int lowerU, int lowerV, int upperU, int upperV) {
    // Compute sum from rolling sum using:
    // sum =
    // + s(upperU,upperV)
    // - s(lowerU,upperV)
  ...
  private double safeSum(double[] rollingSum, int upperU, int upperV) {
    if (upperU < 0 || upperV < 0) {
      return 0;
    }
    upperU = FastMath.min(upperU, psfWidth - 1);
   ...
   */
  private static int findIndex(double[] sum, double pvalue) {
