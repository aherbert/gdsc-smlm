...
public abstract class BlockFilter extends BaseWeightedFilter
{
	...
	private float[] weightedDivisor = null;
	...
	public void rollingBlockFilterInternal(float[] data, final int maxx, final int maxy, final int n)
	{
		...
		{
			if (n == 1)
				rollingBlockFilter3x3InternalWeighted(data, maxx, maxy);
			else
				rollingBlockFilterNxNInternalWeighted(data, maxx, maxy, n);
		}
		else
		{
			// Note: Speed tests show that this method is only marginally faster than rollingBlockFilterNxNInternal.
	...
	void rollingBlockFilterNxNInternal(float[] data, final int maxx, final int maxy, final int n)
	{
		...
		for (int x = n; x < maxx - n; x++)
		{
			...
			data[centreIndex] = getValue(sum, divisor);
	 ...
	 *            The block size
	 */
	void rollingBlockFilterNxNInternalWeighted(float[] data, final int maxx, final int maxy, final int n)
	{
		final int blockSize = 2 * n + 1;
		if (maxx < blockSize || maxy < blockSize)
			return;

		float[] newData = initialiseWeighted(data, maxx, maxy, n);
		...
		for (int x = n; x < maxx - n; x++)
		{
			...
			data[centreIndex] = getValue(sum, weightedDivisor[centreIndex]);
	 ...
	 *            The height of the data
	 */
	void rollingBlockFilter3x3InternalWeighted(float[] data, final int maxx, final int maxy)
	{
		if (maxx < 3 || maxy < 3)
			return;

		float[] newData = initialiseWeighted(data, maxx, maxy, 1);
	...
	public void stripedBlockFilterInternal(float[] data, final int maxx, final int maxy, final int n)
	{
		...
		{
			if (n == 1)
				stripedBlockFilter3x3InternalWeighted(data, maxx, maxy);
			else if (n == 2)
				stripedBlockFilter5x5InternalWeighted(data, maxx, maxy);
			else if (n == 3)
				stripedBlockFilter7x7InternalWeighted(data, maxx, maxy);
			else
				stripedBlockFilterNxNInternalWeighted(data, maxx, maxy, n);
		}
		else
		{
			if (n == 1)
	...
	public void stripedBlockFilterInternal(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		{
			if (w <= 1)
				stripedBlockFilter3x3InternalWeighted(data, maxx, maxy, w);
			else if (w <= 2)
				stripedBlockFilter5x5InternalWeighted(data, maxx, maxy, w);
			else if (w <= 3)
				stripedBlockFilter7x7InternalWeighted(data, maxx, maxy, w);
			else
				stripedBlockFilterNxNInternalWeighted(data, maxx, maxy, w);
		}
		else
		{
			if (w <= 1)
	...
	void stripedBlockFilterNxNInternal(float[] data, final int maxx, final int maxy, final int n)
	{
		...
		for (int x = n; x < maxx - n; x++)
		{
			...
			for (int y = 0; y <= maxy - blockSize; y++, index++)
			{
				...
				data[x + (y + n) * maxx] = getValue(sum, divisor);
			}
		}
	}
	 ...
	 *            The block size
	 */
	void stripedBlockFilterNxNInternalWeighted(float[] data, final int maxx, final int maxy, final int n)
	{
		final int blockSize = 2 * n + 1;
		if (maxx < blockSize || maxy < blockSize)
			return;
		...
		for (int x = n; x < maxx - n; x++)
		{
			...
			for (int y = 0; y <= maxy - blockSize; y++, index++)
			{
				...
				data[index2] = getValue(sum, weightedDivisor[index2]);
			}
		}
	}
	...
	void stripedBlockFilterNxNInternal(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		for (int x = n1; x < maxx - n1; x++)
		{
			...
			for (int y = 0; y < maxy - blockSize; y++, index++)
			{
				...
				data[x + (y + n1) * maxx] = getValue(sum, divisor);
			}
		}
	}
	 ...
	 *            The block size
	 */
	void stripedBlockFilterNxNInternalWeighted(float[] data, final int maxx, final int maxy, final float w)
	{
		final int n = (int) w;
		final int n1 = (n == w) ? n : n + 1;
		...
		float[] newData = initialiseWeighted(data, maxx, maxy, w);
	...
	void stripedBlockFilter3x3Internal(float[] data, final int maxx, final int maxy)
	{
		...
		for (int x = 1; x < maxx - 1; x++)
		{
			...
			{
				data[index2] = getValue(newData[index] + newData[index + 1] + newData[index + 2], divisor);
				index2 += maxx;
			}
		}
	}
	 ...
	 *            The height of the data
	 */
	void stripedBlockFilter3x3InternalWeighted(float[] data, final int maxx, final int maxy)
	{
		if (maxx < 3 || maxy < 3)
			return;
		...
		for (int x = 1; x < maxx - 1; x++)
		{
			...
			for (int y = 0; y <= maxy - 3; y++, index++)
			{
				data[index2] = getValue(newData[index] + newData[index + 1] + newData[index + 2],
						weightedDivisor[index2]);
				index2 += maxx;
			}
		}
	}
	...
	void stripedBlockFilter3x3Internal(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		for (int x = 1; x < maxx - 1; x++)
		{
			...
			for (int y = 0; y <= maxy - 3; y++, index++)
			{
				data[index2] = getValue(w * (newData[index] + newData[index + 2]) + newData[index + 1], divisor);
				index2 += maxx;
			}
		}
	}
	 ...
	 *            The weight
	 */
	void stripedBlockFilter3x3InternalWeighted(float[] data, final int maxx, final int maxy, final float w)
	{
		if (maxx < 3 || maxy < 3)
			return;
		...
		for (int x = 1; x < maxx - 1; x++)
		{
			...
			for (int y = 0; y <= maxy - 3; y++, index++)
			{
				data[index2] = getValue(w * (newData[index] + newData[index + 2]) + newData[index + 1],
						weightedDivisor[index2]);
				index2 += maxx;
			}
		}
		...
		for (int x = 2; x < maxx - 2; x++)
		{
			...
			for (int y = 0; y <= maxy - 5; y++, index++)
			{
				float sum = newData[index] + newData[index + 1] + newData[index + 2] + newData[index + 3] +
						newData[index + 4];
				data[index2] = getValue(sum, divisor);
				index2 += maxx;
			}
		}
	}
	 ...
	 *            The height of the data
	 */
	void stripedBlockFilter5x5InternalWeighted(float[] data, final int maxx, final int maxy)
	{
		if (maxx < 5 || maxy < 5)
			return;

		float[] newData = initialiseWeighted(data, maxx, maxy, 2);
	...
	void stripedBlockFilter5x5Internal(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		for (int x = 2; x < maxx - 2; x++)
		{
			...
			for (int y = 0; y <= maxy - 5; y++, index++)
			{
				data[index2] = getValue(w1 * (newData[index] + newData[index + 4]) + newData[index + 1] +
						newData[index + 2] + newData[index + 3], divisor);
				index2 += maxx;
			}
		}
	}
	 ...
	 *            The weight (should be between 1 and 2)
	 */
	void stripedBlockFilter5x5InternalWeighted(float[] data, final int maxx, final int maxy, final float w)
	{
		if (maxx < 5 || maxy < 5)
			return;
		...
		for (int x = 2; x < maxx - 2; x++)
		{
			...
			{
				data[index2] = getValue(w1 * (newData[index] + newData[index + 4]) + newData[index + 1] +
						newData[index + 2] + newData[index + 3], weightedDivisor[index2]);
				index2 += maxx;
			}
		}
	}
	 ...
	 *            The height of the data
	 */
	void stripedBlockFilter7x7InternalWeighted(float[] data, final int maxx, final int maxy)
	{
		if (maxx < 7 || maxy < 7)
			return;

		float[] newData = initialiseWeighted(data, maxx, maxy, 3);
	...
	void stripedBlockFilter7x7Internal(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		for (int x = 3; x < maxx - 3; x++)
		{
			...
			for (int y = 0; y <= maxy - 7; y++, index++)
			{
				data[index2] = getValue(w1 * (newData[index] + newData[index + 6]) + newData[index + 1] +
						newData[index + 2] + newData[index + 3] + newData[index + 4] + newData[index + 5], divisor);
				index2 += maxx;
			}
		}
	}
	 ...
	 *            The weight (should be between 2 and 3)
	 */
	void stripedBlockFilter7x7InternalWeighted(float[] data, final int maxx, final int maxy, final float w)
	{
		if (maxx < 7 || maxy < 7)
			return;
		...
		for (int x = 3; x < maxx - 3; x++)
		{
			...
			{
				data[index2] = getValue(w1 * (newData[index] + newData[index + 6]) + newData[index + 1] +
						newData[index + 2] + newData[index + 3] + newData[index + 4] + newData[index + 5],
						weightedDivisor[index2]);
				index2 += maxx;
			}
		}
	...
	public void blockFilterInternal(float[] data, final int maxx, final int maxy, final int n)
	{
		...
		{
			if (n == 1)
				blockFilter3x3InternalWeighted(data, maxx, maxy);
			else
				blockFilterNxNInternalWeighted(data, maxx, maxy, n);
		}
		else
		{
			if (n == 1)
	...
	public void blockFilterInternal(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		{
			if (w < 1)
				blockFilter3x3InternalWeighted(data, maxx, maxy, w);
			else
				blockFilterNxNInternalWeighted(data, maxx, maxy, w);
		}
		else
		{
			if (w < 1)
	...
	void blockFilterNxNInternal(float[] data, final int maxx, final int maxy, final int n)
	{
		...
		for (int y = n; y < maxy - n; y++)
		{
			...
			for (int x = n; x < maxx - n; x++, index++)
			{
				...
				newData[index] = getValue(sum, divisor);
			}
		}
	 ...
	 *            The block size
	 */
	void blockFilterNxNInternalWeighted(float[] data, final int maxx, final int maxy, final int n)
	{
		// The size of the region 
		final int nX = (2 * n + 1);
		final int nY = (2 * n + 1);
		...
		for (int y = n; y < maxy - n; y++)
		{
			...
			for (int x = n; x < maxx - n; x++, index++)
			{
				...
				newData[index] = getValue(sum, weightedDivisor[index]);
			}
		}
	...
	void blockFilterNxNInternal(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		final float[] newData = initialise(data.length);
		...
		for (int y = n1; y < ylimit; y++)
		{
			...
			for (int x = n1; x < xlimit; x++, index++)
			{
				...
				newData[index] = getValue(sum + sum1 * w1 + sum2 * w2, divisor);
			}
		}
	 ...
	 *            The block size
	 */
	void blockFilterNxNInternalWeighted(float[] data, final int maxx, final int maxy, final float w)
	{
		final int n = (int) w;
		final int n1 = (n == w) ? n : n + 1;
		...
		final float[] newData = initialiseWeighted(data, maxx, maxy, w);
		...
		for (int y = n1; y < ylimit; y++)
		{
			...
			for (int x = n1; x < xlimit; x++, index++)
			{
				...
				newData[index] = getValue(sum + sum1 * w1 + sum2 * w2, weightedDivisor[index]);
			}
		}
	...
	void blockFilter3x3Internal(float[] data, final int maxx, final int maxy)
	{
		...
		for (int y = 1; y < maxy - 1; y++)
		{
			...
			for (int x = 1; x < maxx - 1; x++)
			{
				...
				newData[index1] = getValue(sum, divisor);
				index0++;
				index1++;
				index2++;
			}
	 ...
	 *            The height of the data
	 */
	void blockFilter3x3InternalWeighted(float[] data, final int maxx, final int maxy)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, 1);
		...
		for (int y = 1; y < maxy - 1; y++)
		{
			...
			for (int x = 1; x < maxx - 1; x++)
			{
				float sum = data[index0 - 1] + data[index0] + data[index0 + 1] + data[index1 - 1] + data[index1] +
						data[index1 + 1] + data[index2 - 1] + data[index2] + data[index2 + 1];
				newData[index1] = getValue(sum, weightedDivisor[index1]);
				index0++;
				index1++;
				index2++;
			}
	...
	void blockFilter3x3Internal(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		for (int y = 1; y < maxy - 1; y++)
		{
			...
			for (int x = 1; x < maxx - 1; x++)
			{
				...
				newData[index1] = getValue(data[index1] + sum1 * w + sum2 * w2, divisor);
				index0++;
				index1++;
				index2++;
			}
	 ...
	 *            The weight
	 */
	void blockFilter3x3InternalWeighted(float[] data, final int maxx, final int maxy, final float w)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, w);
		...
		for (int y = 1; y < maxy - 1; y++)
		{
			...
			for (int x = 1; x < maxx - 1; x++)
			{
				...
				newData[index1] = getValue(data[index1] + sum1 * w + sum2 * w2, weightedDivisor[index1]);
				index0++;
				index1++;
				index2++;
			}
	...
	/**
	 * Initialise for an unweighted filter.
	 *
	 * @param size
	 *            the size
	 * @return the float buffer
	 */
	private float[] initialise(int size)
	{
		if (hasWeights())
	...
	/**
	 * Initialise for a weighted filter.
	 *
	 * @param data
	 *            the data
	 * @param maxx
	 ...
	 * @return the float buffer
	 */
	private float[] initialiseWeighted(float[] data, final int maxx, final int maxy, final float n)
	{
		if (!hasWeights())
	...
	public void rollingBlockFilter(float[] data, final int maxx, final int maxy, final int n)
	{
		...
		{
			if (n == 1)
				rollingBlockFilter3x3Weighted(data, maxx, maxy);
			else
				rollingBlockFilterNxNWeighted(data, maxx, maxy, n);
		}
		else
		{
			if (n == 1)
	...
	void rollingBlockFilterNxN(float[] data, final int maxx, final int maxy, final int n)
	{
		...
		for (int y = 0; y < height; y++)
		{
			...
			int centreIndex = y;
			outData[centreIndex] = getValue(sum, divisor);
	 ...
	 *            The block size
	 */
	void rollingBlockFilterNxNWeighted(float[] data, final int maxx, final int maxy, final int n)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, n);
		...
		for (int y = 0; y < height; y++)
		{
			...
			int centreIndex = y;
			outData[centreIndex] = getValue(sum, weightedDivisor[centreIndex]);
	 ...
	 *            The height of the data
	 */
	void rollingBlockFilter3x3Weighted(float[] data, final int maxx, final int maxy)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, 1);
	...
	public void stripedBlockFilter(float[] data, final int maxx, final int maxy, final int n)
	{
		...
		{
			if (n == 1)
				stripedBlockFilter3x3Weighted(data, maxx, maxy);
			else if (n == 2)
				stripedBlockFilter5x5Weighted(data, maxx, maxy, n);
			else if (n == 3)
				stripedBlockFilter7x7Weighted(data, maxx, maxy, n);
			else
				stripedBlockFilterNxNWeighted(data, maxx, maxy, n);
		}
		else
		{
			if (n == 1)
				...
				stripedBlockFilter5x5(data, maxx, maxy);
			else if (n == 3)
				stripedBlockFilter7x7(data, maxx, maxy);
			else
	...
	public void stripedBlockFilter(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		{
			if (w <= 1)
				stripedBlockFilter3x3Weighted(data, maxx, maxy, w);
			else if (w <= 2)
				stripedBlockFilter5x5Weighted(data, maxx, maxy, w);
			else if (w <= 3)
				stripedBlockFilter7x7Weighted(data, maxx, maxy, w);
			else
				stripedBlockFilterNxNWeighted(data, maxx, maxy, w);
		}
		else
		{
			if (w <= 1)
	 ...
	 *            The block size
	 */
	void stripedBlockFilterNxNWeighted(float[] data, final int maxx, final int maxy, final int n)
	{
		int blockSize = 2 * n + 1;
	 ...
	 *            The block size
	 */
	void stripedBlockFilterNxNWeighted(float[] data, final int maxx, final int maxy, final float w)
	{
		final int n = (int) w;
		final int n1 = (n == w) ? n : n + 1;
	 ...
	 *            The height of the data
	 */
	void stripedBlockFilter3x3Weighted(float[] data, final int maxx, final int maxy)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, 1);
	...
	void stripedBlockFilter3x3(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		for (int y = 0; y < height; y++)
		{
			...
			for (int x = 0; x < width; x++)
			{
				// Sum strips
				// Store result in transpose
				outData[centreIndex] = getValue(w * (row[x] + row[x + 2]) + row[x + 1], divisor);
				centreIndex += height;
			}
		}
	}
	 ...
	 *            The weight
	 */
	void stripedBlockFilter3x3Weighted(float[] data, final int maxx, final int maxy, final float w)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, w);
		...
		for (int y = 0; y < height; y++)
		{
			...
			for (int x = 0; x < width; x++)
			{
				// Sum strips
				// Store result in transpose
				outData[centreIndex] = getValue(w * (row[x] + row[x + 2]) + row[x + 1], weightedDivisor[centreIndex]);
				centreIndex += height;
			}
		}
	}
	...
	void stripedBlockFilter5x5(float[] data, final int maxx, final int maxy)
	{
		...
		for (int y = 0; y < height; y++)
		{
			...
			for (int x = 0; x < width; x++)
			{
				// Sum strips
				// Store result in transpose
				outData[centreIndex] = getValue(row[x] + row[x + 1] + row[x + 2] + row[x + 3] + row[x + 4], divisor);
				centreIndex += height;
			}
		}
	}
	 ...
	 *            The height of the data
	 */
	void stripedBlockFilter5x5Weighted(float[] data, final int maxx, final int maxy)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, 2);
		...
		for (int y = 0; y < height; y++)
		{
			...
			for (int x = 0; x < width; x++)
			{
				// Sum strips
				// Store result in transpose
				outData[centreIndex] = getValue(row[x] + row[x + 1] + row[x + 2] + row[x + 3] + row[x + 4],
						weightedDivisor[centreIndex]);
				centreIndex += height;
			}
		}
	}
	...
	void stripedBlockFilter5x5(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		for (int y = 0; y < height; y++)
		{
			...
			for (int x = 0; x < width; x++)
			{
				// Sum strips
				// Store result in transpose
				outData[centreIndex] = getValue(w1 * (row[x] + row[x + 4]) + row[x + 1] + row[x + 2] + row[x + 3],
						divisor);
				centreIndex += height;
			}
		}
	 ...
	 *            The weight (should be between 1 and 2)
	 */
	void stripedBlockFilter5x5Weighted(float[] data, final int maxx, final int maxy, final float w)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, w);
	...
	void stripedBlockFilter7x7(float[] data, final int maxx, final int maxy)
	{
		...
		for (int y = 0; y < height; y++)
		{
			...
			for (int x = 0; x < width; x++)
			{
				// Sum strips
				// Store result in transpose
				outData[centreIndex] = getValue(
						row[x] + row[x + 1] + row[x + 2] + row[x + 3] + row[x + 4] + row[x + 5] + row[x + 6], divisor);
				centreIndex += height;
			}
		}
	}
	 ...
	 *            The height of the data
	 */
	void stripedBlockFilter7x7Weighted(float[] data, final int maxx, final int maxy)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, 3);
	 ...
	 *            The weight (should be between 2 and 3)
	 */
	void stripedBlockFilter7x7Weighted(float[] data, final int maxx, final int maxy, final float w)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, w);
	...
	public void blockFilter(float[] data, final int maxx, final int maxy, final int n)
	{
		...
		{
			if (n == 1)
				blockFilter3x3Weighted(data, maxx, maxy);
			else
				blockFilterNxNWeighted(data, maxx, maxy, n);
		}
		else
		{
			if (n == 1)
	...
	public void blockFilter(float[] data, final int maxx, final int maxy, final float w)
	{
		...
		{
			if (w < 1)
				blockFilter3x3Weighted(data, maxx, maxy, w);
			else
				blockFilterNxNWeighted(data, maxx, maxy, w);
		}
		else
		{
			if (w < 1)
	 ...
	 *            The block size
	 */
	void blockFilterNxNWeighted(float[] data, final int maxx, final int maxy, final int n)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, n);
	 ...
	 *            The block size
	 */
	void blockFilterNxNWeighted(float[] data, final int maxx, final int maxy, final float w)
	{
		final int n = (int) w;
		final int n1 = (n == w) ? n : n + 1;
	 ...
	 *            The height of the data
	 */
	void blockFilter3x3Weighted(float[] data, final int maxx, final int maxy)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, 1);
	 ...
	 *            The weight
	 */
	void blockFilter3x3Weighted(float[] data, final int maxx, final int maxy, final float w)
	{
		float[] newData = initialiseWeighted(data, maxx, maxy, w);
