 ...
 * Poisson-Gaussian distribution. <p> For each observed value the log-likelihood is computed from
 * the Poisson-Gaussian distribution (a Poisson convolved with a Gaussian). The mean of the Poisson
 * distribution is set using the expected value generated by the provided function. The standard
 * deviation of the Gaussian is fixed and set in the constructor. The mean of the Gaussian is
 * assumed to be zero. <p> The negative log-likelihood can be evaluated over the entire set of
 * observed values or for a chosen observed value. The sum uses a non-normalised Poisson-Gaussian
 * distribution for speed (see
