...
public class CircularGaussian2DFunction extends MultiPeakGaussian2DFunction {
  ...
  protected static final int GRADIENT_PARAMETERS_PER_PEAK = 4;
   ...
   *        coordinates)
   */
  public CircularGaussian2DFunction(int numberOfPeaks, int maxx, int maxy) {
    super(numberOfPeaks, maxx, maxy);
    peakFactors = new double[numberOfPeaks][5];
  }
  ...
  /**
   * Evaluates a 2-dimensional circular Gaussian function for multiple peaks.
   *
   * <p>{@inheritDoc}
   */
  @Override
  ...
  protected double gaussian(final int x0, final int x1, final double[] dyDa, final int apos,
    ...
    dyDa[dydapos + 3] = factors[AX] * y * (1 + aadx2dy2);
