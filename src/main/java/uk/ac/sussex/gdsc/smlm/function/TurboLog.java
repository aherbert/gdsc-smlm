...
public class TurboLog extends FastLog {
   ...
   * relative error of 7.95e-4 for x below 1.
   */
  public static final double LOWER_ONE_BOUND = 0.92;
  /**
   * The bounds above 1 where the function switches to use Math.log. This results in a maximum
   * relative error of 7.09e-4 for x above 1.
   */
  ...
  static {
    ...
    assert ((Double.doubleToRawLongBits(LOWER_ONE_BOUND) >> 52)
  ...
  /**
   * The table of the log value of the floating point mantissa (a binary number 1.0000... to
   ...
   * values for base E and the default table size.
   */
  public TurboLog() {
    this(N);
  }
  ...
  /**
   * Create a new natural logarithm calculation instance. This will hold the pre-calculated log
   * values for base E and a table size depending on a given mantissa precision.
   *
  ...
  public TurboLog(int n) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      // assert logv == fastLog(value) : String.format("[%d] data[i](%g) %g != %g %g", i, value,
      // logv,
      // fastLog2(value), uk.ac.sussex.gdsc.core.utils.FloatEquality.relativeError(logv,
      // fastLog2(value)));
    }
  }
