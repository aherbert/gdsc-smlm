...
/**
 * Fits local maxima using a 2D Gaussian. <p> Multi-threaded for speed. Uses a
 * BlockingQueue to hold the ImageProcessor work which is then processed
 * sequentially by worker threads. The queue behaviour when the size is much
 * greater than the number of worker threads can be configured.
 */
public class FitEngine {
    ...
    /**
     * Return the fitting window size calculated using the fitting parameter and the
     * configured peak widths. The actual window is 2n+1 around the local maxima.
     * 
    ...
    public FitEngine(FitEngineConfiguration config, PeakResults results, int threads, FitQueue queueType,
        ...
        // String.format("/tmp/%s%s.log",
        //// config.getFitConfiguration().getFitSolver().getShortName(),
    ...
    /**
     * Locate all the peaks in the given processor. Adds the work to the current
     * queue.
     * 
     * @param job The job
     */
    public void run(FitJob job) {
        ...
        // Check the output is still OK. If no output then there is no point running any
        // calculations.
        if (results.isActive()) {
    ...
    /**
     * Signal that no more fitting work will be added to the queue. <p> Ask all
     * threads to end and wait. Returns when all threads have stopped running.
     * 
    ...
    public synchronized void end(boolean now) {
        ...
        if (now) {
            ...
            // Add null jobs if the queue is not at capacity so they can be collected by
            // alive workers.
            // If there are already jobs then the worker will stop due to the finish()
            // signal.
