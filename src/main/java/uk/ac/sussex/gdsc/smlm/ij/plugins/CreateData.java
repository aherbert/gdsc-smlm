...
public class CreateData implements PlugIn, ItemListener, RandomGeneratorFactory
{
    ...
    public static class BenchmarkParameters extends BaseParameters
    {
        ...
        public void setPhotons(MemoryPeakResults results)
        {
            ...
            ImageJUtils.log(
                    ...
                    frames, molecules, MathUtils.rounded(averageSignal), MathUtils.rounded(av), MathUtils.rounded(b),
                    MathUtils.rounded(av2));
    ...
    public void run(String arg)
    {
        ...
        if (simpleMode || benchmarkMode || spotMode)
        {
            ...
            while (id < settings.getParticles())
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    ...
                    final LocalisationModel m = new LocalisationModel(id, t, xyz, intensity,
                            LocalisationModel.CONTINUOUS);
                    localisations.add(m);

                    // Each localisation can be a separate localisation set
                    final LocalisationModelSet set = new LocalisationModelSet(id, t);
                    set.add(m);
        ...
        else
        {
            ...
            else
            {
                ...
                final SpatialIllumination activationIllumination = createIllumination(settings.getPulseRatio(),
            ...
            if (compounds == null)
                return;
            final List<CompoundMoleculeModel> molecules = imageModel.createMolecules(compounds, settings.getParticles(),
    ...
    private void reportAndSaveFittingLimits(SpatialDistribution dist)
    {
        ...
        ImageJUtils.log("Signal (N) = %s - %s photons", MathUtils.rounded(settings.getPhotonsPerSecond()),
        ...
        ImageJUtils.log("Effective background noise = %s photo-electron [includes read noise and background photons]",
        ...
        ImageJUtils.log("Localisation precision (LSE): %s - %s nm : %s - %s px", MathUtils.rounded(lowerP), MathUtils.rounded(upperP),
                MathUtils.rounded(lowerP / a), MathUtils.rounded(upperP / a));
        ImageJUtils.log("Localisation precision (MLE): %s - %s nm : %s - %s px", MathUtils.rounded(lowerMLP),
                MathUtils.rounded(upperMLP), MathUtils.rounded(lowerMLP / a), MathUtils.rounded(upperMLP / a));
        ...
        if (min == max)
        {
            ...
            final UnivariateLikelihoodFisherInformationCalculator c = new UnivariateLikelihoodFisherInformationCalculator(
                    f, fiFunction);
            ...
            if (crlb != null)
            {
                ImageJUtils.log("Localisation precision (CRLB): B=%s, I=%s photons", MathUtils.rounded(crlb[0]),
                        MathUtils.rounded(crlb[1]));
                ImageJUtils.log("Localisation precision (CRLB): X=%s, Y=%s, Z=%s nm : %s,%s,%s px",
                        MathUtils.rounded(crlb[2] * a), MathUtils.rounded(crlb[3] * a), MathUtils.rounded(crlb[4] * a),
                        MathUtils.rounded(crlb[2]), MathUtils.rounded(crlb[3]), MathUtils.rounded(crlb[4]));
            }
        ...
        else
        {
            ...
            ImageJUtils.log("50%% PSF SNR : %s - %s : Peak SNR : %s - %s", MathUtils.rounded(u / noise),
                    MathUtils.rounded(scale * u / noise), MathUtils.rounded(u0 / noise), MathUtils.rounded(scale * u0 / noise));
            ImageJUtils.log(
    ...
    private double getNoiseEstimateInPhotoelectrons(double qe)
    {
        if (CalibrationProtosHelper.isCCDCameraType(settings.getCameraType()))
        {
            ...
            if (settings.getCameraType() == CameraType.EMCCD && settings.getEmGain() > 1)
            {
                ...
                // See uk.ac.sussex.gdsc.smlm.results.Gaussian2DPeakResultHelper.getPrecision(double, double, double, double, boolean)
    ...
    private int checkTotalSteps(int totalSteps, List<? extends FluorophoreSequenceModel> fluorophores)
    {
        ...
        if (max > totalSteps)
        {
            ...
            if (totalSteps != 0)
            {
                final int totalFrames = (int) Math.ceil(settings.getSeconds() * 1000 / settings.getExposureTime());
    ...
    private void updateArea(int[] mask, int w, int h)
    {
        ...
        // Threshold to a mask
        final FloatProcessor fp = new FloatProcessor(w, h, pixels);
    ...
    private UniformDistribution createUniformDistribution(double border)
    {
        ...
        final double[] max = new double[] { settings.getSize() / 2 - border, settings.getSize() / 2 - border, depth };
    ...
    private RealDistribution createPhotonDistribution()
    {
            ...
            if (filename != null)
            {
                        ...
                        new UnicodeReader(new FileInputStream(new File(settings.getPhotonDistributionFile())), null)))
                {
                    ...
                    if (stats.getSum() > 0)
                    {
                        ...
                        final EmpiricalDistribution dist = new EmpiricalDistribution(binCount, createRandomGenerator());
                        dist.load(values);
                        return dist;
                    }
                }
        ...
        else if (PHOTON_DISTRIBUTION[PHOTON_GAMMA].equals(settings.getPhotonDistribution()))
        {
            ...
            final GammaDistribution dist = new GammaDistribution(createRandomGenerator(), settings.getPhotonShape(),
    ...
    private List<LocalisationModelSet> combineSimulationSteps(List<LocalisationModel> localisations)
    {
        ...
        final List<LocalisationModelSet> newLocalisations = new ArrayList<>(
    ...
    private class ImageGenerator implements Runnable
    {
        ...
        public void run()
        {
            ...
            final int fromIndex = findIndexByTime(localisations, startIndex, t);
            if (fromIndex > -1 && psfModel != null)
            {
                final int toIndex = findLastIndexByTime(localisations, fromIndex, t);
                ...
                for (final LocalisationModelSet localisationSet : subset)
                {
                    ...
                    final float[] params = Gaussian2DPeakResultHelper.createTwoAxisParams((float) backgroundInPhotons,
                            intensity, x, y, z, sx, sy);
                    final float u = (float) Gaussian2DPeakResultHelper.getMeanSignalUsingP05(intensity, sx, sy);
        ...
        private void amplify(float[] image)
        {
            ...
            if (emGain != 0)
            {
                ...
                else
                {
                    ...
                    final CustomGammaDistribution dist = new CustomGammaDistribution(random.getRandomGenerator(), shape,
    ...
    private float[] createBackground(RandomDataGenerator random)
    {
        ...
        if (settings.getBackground() > 0)
        {
            ...
            if (uniformBackground)
            {
                ...
                final UniformIntegerDistribution d = new UniformIntegerDistribution(random.getRandomGenerator(), 0,
                        upper);
                while (samples-- > 0)
                    pixels2[d.sample()] += 1;
            ...
            {
                final CustomPoissonDistribution dist = new CustomPoissonDistribution(random.getRandomGenerator(), 1,
                        PoissonDistribution.DEFAULT_EPSILON, PoissonDistribution.DEFAULT_MAX_ITERATIONS);
                for (int i = 0; i < pixels2.length; i++)
            ...
            List<LocalisationModel> localisations)
    {
        ...
        final int[] countHistogram = new int[nFrames + 1];
    ...
    private void saveImageResults(MemoryPeakResults results)
    {
        ...
        if (chooser.getFileName() != null)
        {
            ...
            settings.setImageResultsFilename(ImageJUtils.replaceExtension(settings.getImageResultsFilename(), "xls"));
        ...
        for (final LocalisationModel localisation : localisations)
        {
            if (currentId != localisation.getId() || lastT + 1 != localisation.getTime())
            {
                if (n > 0)
                {
                    ...
                    final ExtendedPeakResult p = new ExtendedPeakResult(start.getTime(), (int) Math.round(start.getX()),
    ...
    private void saveFluorophores(List<? extends FluorophoreSequenceModel> fluorophores)
    {
        ...
        if (chooser.getFileName() != null)
        {
            ...
            settings.setFluorophoresFilename(ImageJUtils.replaceExtension(settings.getFluorophoresFilename(), "xls"));
    ...
    private boolean showDialog()
    {
        ...
        final String[] dist = (trackMode) ? Arrays.copyOf(PHOTON_DISTRIBUTION, PHOTON_DISTRIBUTION.length - 1)
                : PHOTON_DISTRIBUTION;
    ...
    private List<CompoundMoleculeModel> createCompoundMolecules()
    {
        ...
        if (settings.getCompoundMolecules())
            ...
            try
            {
                ...
                for (final Molecule m : builder.getMoleculeList())
                {
                    ...
                    compounds.add(cm);
                    compoundNames.add(String.format("Fraction=%s, D=%s um^2/s", MathUtils.rounded(cm.getFraction()),
                            MathUtils.rounded(m.getDiffusionRate())));
                }
        ...
        else
        {
            ...
            final CompoundMoleculeModel m = new CompoundMoleculeModel(1, 0, 0, 0,
    ...
    private SimulationParameters showSimulationParametersDialog(ImagePlus imp, MemoryPeakResults results)
    {
        ...
        final double backgroundVariance = b;
        ...
        final SimulationParameters p = new SimulationParameters(molecules, fullSimulation, s, a, minSignal, maxSignal,
