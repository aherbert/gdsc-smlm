...
class CandidateGridManager {
  ...
  /**
   * Add a fittedCandidate to the grid. Assumes that the coordinates are within the size of the
   * grid.
   *
  ...
  public CandidateList getFittedNeighbours(final int x, final int y) {
    ...
    if (fittedCandidateCache != null && fittedCandidateCacheX == xBlock
   ...
   *
   * @param candidate the candidate
   * @param nonUnique True if candidate indices are not unique
   * @return the neighbours
   */
