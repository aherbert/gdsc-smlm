...
public class PeakFit implements PlugInFilter, ItemListener
{
    ...
    public int setup(String arg, ImagePlus imp)
    {
        ...
        else
        {
            ...
            if (imp.getTitle().endsWith(IJImagePeakResults.IMAGE_SUFFIX))
            {
                ...
                final Pattern pattern = Pattern.compile("Source: (<.*IJImageSource>.*<.*IJImageSource>)",
                        Pattern.DOTALL);
                ...
                CalibrationProtosHelper.getName(calibration.getCameraType()), new OptionListener<Integer>()
                {
                    ...
                    public boolean collectOptions(Integer field)
                    {
                        final CalibrationWriter calibration = new CalibrationWriter(
                                calibrationProvider.getCalibration());
            ...
            int flags)
    {
        ...
        if (fitSolver == FitSolver.MLE)
        {
            ...
            final String[] searchNames = SettingsManager
                    .getNames((Object[]) MaximumLikelihoodFitter.SearchMethod.values());
    ...
    {
        final IJTablePeakResults r = ResultsManager.addTableResults(resultsList,
                resultsSettings.getResultsTableSettings(), resultsSettings.getShowDeviations(), false, false, false);
        if (r != null)
        {
