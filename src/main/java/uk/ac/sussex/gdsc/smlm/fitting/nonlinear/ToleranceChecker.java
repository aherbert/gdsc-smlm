...
public class ToleranceChecker {
  ...
  /**
   * Flag indicating if the parameters will be checked for convergence. Either
   * {@link #relativeParameters} or {@link #absoluteParameters} will be above zero.
   */
  public final boolean checkParameters;
  /**
   * Flag indicating that the value must be minimised. Convergence is only signalled on the value
   * with a marginal improvement in the value with the direction being either minimal or maximal,
   * i.e. if the value becomes worse but the change is within the tolerance then convergence will
   * not be signalled.
   */
  private boolean minimiseValue;
  /**
   * The maximum number of allowed iterations. Set above zero to limit the iterations. Set below
   * zero to define a specific number of iterations until convergence is signalled. Set to zero to
   * disable.
   */
   ...
   *        disable.
   * @param absoluteParameters absolute tolerance threshold on the parameters. Set to negative to
   *        disable.
   * @throws IllegalArgumentException if none of the convergence criteria are valid (i.e.
   *         convergence is not possible)
   */
  public ToleranceChecker(double relativeParameters, double absoluteParameters) {
    this(false, IGNORE_TOLERANCE, IGNORE_TOLERANCE, relativeParameters, absoluteParameters,
        IGNORE_MAX_ITERATIONS);
  }
   ...
   * @param absoluteParameters absolute tolerance threshold on the parameters. Set to negative to
   *        disable.
   * @param maxIterations The maximum number of allowed iterations. Set above zero to limit the
   *        iterations. Set below zero to define a specific number of iterations until convergence
   *        is signalled. Set to zero to disable.
   * @throws IllegalArgumentException if none of the convergence criteria are valid (i.e.
   *         convergence is not possible)
   */
  ...
  public int converged(double previousValue, double[] previousParameters, double currentValue,
    ...
    if (checkParameters && areEqual(previousParameters, currentParameters, absoluteParameters,
        relativeParameters)) {
  ...
  /**
   * Sets the flag indicating that the value must be minimised. Convergence is only signalled on the
   * value with a marginal improvement in the value with the direction being either minimal or
   * maximal, i.e. if the value becomes worse but the change is within the tolerance then
   * convergence will not be signalled.
   *
   * @param minimiseValue true, if the value must be minimised
   */
  ...
  /**
   * Sets the converged flag to true. This can be used to manually set convergence, for example in
   * the event that further computation is not needed.
   */
