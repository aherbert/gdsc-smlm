...
public class DFastLog
{
	 ...
	 *            requires 32 KB.
	 */
	public DFastLog(double base, int q)
	{
		...
		if (q < 0 || q > 30)
			throw new IllegalArgumentException("Q must be in the range 0<=q<=30");

		final int p = 1 << (31 - q);

		// Adjust q with the extra precision of a double mantissa = (52-23)=29
		q += 29;
		...
		for (int i = 0; i < p; i++)
		{
			...
			// int m = i << q
			// int e = 0
			// log2(m x 2^(e-1075)) == log2(m) + log2(2^-1075) = log2(m) -1075 

			data[i] = (float) (log2(((long) i) << q) - 1075);
		}
	}
	...
	// Note: log(float) is not provided as it is dynamically cast up to a double 
	// and float values can be represented as a double. 
}
