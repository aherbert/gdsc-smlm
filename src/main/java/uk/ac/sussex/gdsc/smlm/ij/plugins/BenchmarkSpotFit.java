...
public class BenchmarkSpotFit implements PlugIn, ItemListener
{
            ...
            if (actual.length > 0)
            {
                ...
                if (!predicted.isEmpty())
                {
                    ...
                    for (int jj = 0; jj < predicted.size(); jj++)
                    {
                        ...
                        {
                            final BasePreprocessedPeakResult result = (BasePreprocessedPeakResult) predicted
                                    .get(jj).result;
            ...
            final PreprocessedPeakResult[] preprocessedPeakResults, int nUniqueIDs)
    {
        ...
        final MultiPathFitResults[] multiResults = multiPathResults
                .toArray(new MultiPathFitResults[multiPathResults.size()]);
            ...
            double[][] matchScores)
    {
        ...
        if (plot != null)
        {
            ...
            if (requireLabel)
            {
                final String label = String.format("Max+ %s @ %s, Max- %s @ %s", Utils.rounded(max1),
                        Utils.rounded(maxx1), Utils.rounded(max2), Utils.rounded(maxx2));
                plot.setColor(Color.black);
                plot.addLabel(0, 0, label);
            }
            final PlotWindow pw = Utils.display(title, plot);
