 ...
 * <p>
 * See Ulbrich &amp; Isacoff (2007) Subunit counting in membrane-bound proteins. Nature Methods 4, 319-321
 * (Supplementary
 * Information).
 */
public class EMGainAnalysis implements PlugInFilter
{
    ...
    private void fit(int[] h)
    {
        ...
        for (int iter = 0; iter < 3; iter++)
        {
            ...
            try
            {
                ...
                final PointValuePair optimum = o.optimize(new MaxEval(maxEval), new ObjectiveFunction(fun),
            ...
            try
            {
                ...
                final MultivariateFunction fun = getFunction(limits, y, max, maxEval);
                final MultivariateFunctionMappingAdapter adapter = new MultivariateFunctionMappingAdapter(fun, lower,
                        upper);
    ...
    private void plotPMF()
    {
        ...
        final String label = String.format("Gain=%s, noise=%s, photons=%s", MathUtils.rounded(_gain), MathUtils.rounded(_noise),
                MathUtils.rounded(_photons));
