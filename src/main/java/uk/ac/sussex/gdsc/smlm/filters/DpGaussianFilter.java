 ...
 *
 * <p>This is identical to the {@link GaussianFilter} but the convolution is performed using
 * {@code double} instead of {@code float}, i.e. it is double precision (DP). Intermediate results
 * are stored as {@code float} precision. For a full implementation using {@code double} use
 * {@link DoubleGaussianFilter}.
 */
public class DpGaussianFilter extends BaseWeightedFilter {
  ...
  private void updateWeightedNormaliser(final double sigmaX, final double sigmaY) {
    ...
    if (normaliser == null || sx != sigmaX || sy != sigmaY) {
      ...
      sx = sigmaX;
      sy = sigmaY;
      final DpGaussianFilter gf = new DpGaussianFilter(accuracy);
   ...
   */
  public DpGaussianFilter(double accuracy) {
   ...
   */
  protected DpGaussianFilter(DpGaussianFilter source) {
    super(source);
   ...
   * @return the copy
   */
  public DpGaussianFilter copy() {
    return new DpGaussianFilter(this);
  }
  ...
  private void blur1Direction(final float[] pixels, Rectangle roi, final int width,
    ...
    // for this: Downscaling gives std devation sigma = 1/sqrt(3); upscale gives sigma = 1/2 (in
    ...
    final int maxLength = doDownscaling
        ...
        : length;
    final double[][] gaussKernel = makeGaussianKernel(sigmaGauss, maxLength);
  ...
  private static final void upscaleLine(final float[] cache, final float[] pixels,
    ...
    for (int xout = writeFrom; xout < writeTo; xout++, pi += pointInc) {
      ...
      pixels[pi] = (float) (cache[xin - 2] * kernel[x] + cache[xin - 1] * kernel[x + reduceBy]
  ...
  private static final void convolveLine(final float[] input, final float[] pixels,
    ...
    final double[] kern = kernel[0]; // the kernel itself
    final double kern0 = kern[0];
