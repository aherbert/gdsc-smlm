...
public class FIRE implements PlugIn {
  ...
  private static UnivariatePointValuePair findMin(UnivariatePointValuePair current,
      UnivariateOptimizer o, UnivariateFunction f, double qValue, double factor) {
    try {
  ...
  private static UnivariatePointValuePair findMin(UnivariatePointValuePair current,
      SimplexOptimizer o, MultivariateFunction f, double qValue) {
    try {
      final NelderMeadSimplex simplex = new NelderMeadSimplex(1);
      final double[] initialSolution = {qValue};
      final PointValuePair solution = o.optimize(new MaxEval(1000),
          new InitialGuess(initialSolution), simplex, new ObjectiveFunction(f), GoalType.MINIMIZE);
  ...
  private static PointValuePair findMin(PointValuePair current, SimplexOptimizer o,
      MultivariateFunction f, double[] initialSolution) {
    try {
      final NelderMeadSimplex simplex = new NelderMeadSimplex(initialSolution.length);
      final PointValuePair next = o.optimize(new MaxEval(1000), new InitialGuess(initialSolution),
          simplex, new ObjectiveFunction(f), GoalType.MINIMIZE);
      if (next == null) {
        return current;
      }
  ...
  private class MultiPlateauness implements MultivariateFunction {
     ...
     *
     * @param frcnum the scaled FRC numerator
     * @param q the precomputed exponential decay (hq)
     * @param low the lower bound of the array for optimisation
     * @param high the higher bound of the array for optimisation
     */
