...
import uk.ac.sussex.gdsc.smlm.ij.frc.FRC.FIREResult;
...
public class FIRE implements PlugIn
{
	...
	public void run(String arg)
	{
		...
		if (this.results2 != null)
		{
			...
			final FireResult result = calculateFireNumber(fourierMethod, samplingMethod, thresholdMethod, fourierImageScale,
					imageSize);
	...
	public FireImages createImages(double fourierImageScale, int imageSize, boolean useSignal)
	{
		...
		else
		{
			...
			for (final int index : indices)
			{
				...
				final IJImagePeakResults image = image1;
				image1 = image2;
				image2 = image;
			...
			double fourierImageScale, int imageSize)
	{
		...
		for (int t = step; t <= maxT - step; t += step)
		{
			...
			final FireResult result = f.calculateFireNumber(fourierMethod, samplingMethod, thresholdMethod, fourierImageScale,
					imageSize);
		...
		final Plot2 plot = new Plot2(title, "Frames", "Resolution (" + units + ")", (float[]) null, (float[]) null);
			...
			ThresholdMethod thresholdMethod, FireImages images)
	{
		...
		final FIREResult result = FRC.calculateFire(frcCurve, thresholdMethod);
		if (result == null)
	...
	private void runQEstimation()
	{
		...
		if (fitPrecision)
		{
			...
			if (sampleDecay)
			{
				...
				final double meanSumOfSquares = (precision.getSumOfSquares() / (images.nmPerPixel * images.nmPerPixel)) /
						precision.getN();
			...
			final double[] initial = new double[] { histogram.mean / images.nmPerPixel, histogram.sigma / images.nmPerPixel,
					qValue };
			...
			if (p != null)
			{
				final double[] point = p.getPointRef();
	...
	{
		final double[] hq = FRC.computeHq(q, mean, sigma);
		final double[] exp_decay = new double[q.length];
		exp_decay[0] = 1;
		for (int i = 1; i < q.length; i++)
		{
			final double sinc_q = sinc(Math.PI * q[i]);
			...
			UnivariateFunction f, double qValue, double factor)
	{
		try
		{
			...
			final UnivariatePointValuePair next = o.optimize(GoalType.MINIMIZE, new MaxEval(3000),
			...
			MultivariateFunction f, double qValue)
	{
		try
		{
			...
			final PointValuePair solution = o.optimize(new MaxEval(1000), new InitialGuess(initialSolution), simplex,
			...
			double[] initialSolution)
	{
		try
		{
			...
			final PointValuePair next = o.optimize(new MaxEval(1000), new InitialGuess(initialSolution), simplex,
					new ObjectiveFunction(f), GoalType.MINIMIZE);
			if (next == null)
				return current;
	...
	private class MultiPlateauness implements MultivariateFunction
	{
		...
		MultiPlateauness(double[] frcnum, double[] q, int low, int high)
		{
			...
			{
				final int index = i + low;
				final double sinc_q = (index == 0) ? 1 : sinc(Math.PI * q[index]);
	...
	private boolean showQEstimationInputDialog()
	{
		...
		final String[] thresholdMethodNames = SettingsManager.getNames((Object[]) FRC.ThresholdMethod.values());
		gd.addChoice("Threshold_method", thresholdMethodNames, thresholdMethodNames[thresholdMethodIndex]);
		final String[] precisionMethodNames = SettingsManager.getNames((Object[]) PrecisionMethod.values());
	...
	private class QPlot
	{
		...
		PlotWindow[] plot(double mean, double sigma, double qValue)
		{
			...
			final double[] hq = FRC.computeHq(q, mu, sd);
			final double[] correction = new double[hq.length];
			final double[] vq_corr = new double[vq.length];
			...
			if (result != null)
			{
				...
				final FrcCurve curve = new FrcCurve();
				final FireResult fireResult = new FireResult(fireNumber, result.correlation, smoothedFrcCurve,
		...
		{
			final double[] exp_decay = computeExpDecay(mu, sd, q);
	...
	private class PrecisionHistogram
	{
		...
		PlotWindow plot()
		{
			...
			if (x != null)
			{
				...
				final double integral = 0.5 * Erf.erf((x2[0] - mean) * denom0, (x2[x2.length - 1] - mean) * denom0);
	...
	private PrecisionHistogram calculatePrecisionHistogram()
	{
		...
		else precision.add(pp.precision);
