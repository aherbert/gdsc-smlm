...
public class CustomPoissonDistribution extends AbstractIntegerDistribution {
  ...
  private double mean;
  /**
   * Maximum number of iterations for cumulative probability. Cumulative probabilities are estimated
   * using either Lanczos series approximation of
   * {@link Gamma#regularizedGammaP(double, double, double, int)} or continued fraction
   * approximation of {@link Gamma#regularizedGammaQ(double, double, double, int)}.
   */
   ...
   * @since 2.1
   */
  public CustomPoissonDistribution(double p, double epsilon, int maxIterations)
   ...
   * @throws NotStrictlyPositiveException if {@code p <= 0}.
   */
  public CustomPoissonDistribution(RandomGenerator rng, double p)
      throws NotStrictlyPositiveException {
  ...
  /**
   * Calculates the Poisson distribution function using a normal approximation. The
   * {@code N(mean, sqrt(mean))} distribution is used to approximate the Poisson distribution. The
   * computation uses "half-correction" (evaluating the normal distribution function at
   * {@code x + 0.5}).
   *
   ...
   * of Poisson Random Variables</i><br> <strong>Computing</strong> vol. 26 pp. 197-207.<br>
   * </blockquote> </li> </ul>
   *
  ...
  private long nextPoisson(double meanPoisson) {
    ...
    for (;;) {
      ...
      if (v < y * logLambda - CombinatoricsUtils.factorialLog((int) (y + lambda))
          + logLambdaFactorial) {
        y = lambda + y;
        break;
      }
