...
public class GaussianFit implements ExtendedPlugInFilter, DialogListener
{
    ...
    private void runFinal(ImageProcessor ip)
    {
        ...
        if (!singleFit)
        {
            ...
            if (params != null)
            {
                ...
                for (int i = 1, n = 0; i < params.length; i += Gaussian2DFunction.PARAMETERS_PER_PEAK, n++)
                {
                    ...
                    addResult(bounds, regionBounds, peakParams, peakParamsDev, nMaxima, x, y, data[maxIndices[n]]);
                ...
                if (showFit && validPeaks != 0)
                {
                    ...
                    final EllipticalGaussian2DFunction f = new EllipticalGaussian2DFunction(validPeaks, width, height);
        ...
        else
        {
            ...
            for (int n = 0; n < maxIndices.length; n++)
            {
                ...
                final double[] peakParams = fitSingle(gf, region, regionBounds.width, regionBounds.height, newIndex,
    ...
    private void addResult(Rectangle bounds, Rectangle regionBounds, double[] params, double[] paramsDev, int n, int x,
            int y, float value)
    {
        x += bounds.x;
        y += bounds.y;
