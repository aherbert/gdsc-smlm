...
public class FitWorker implements Runnable, IMultiPathFitResults, SelectedResultStore
{
    ...
    public void run(FitJob job)
    {
        ...
        else
        {
            ...
            if (slice == -1)
            {
                ...
                FileUtils.save(String.format("/tmp/filter.%d.xml", slice), filter.toXML());
                //filter.setDebugFile(String.format("/tmp/fitWorker.%b.txt", benchmarking));
                final StringBuilder sb = new StringBuilder();
                sb.append((benchmarking) ? ((uk.ac.sussex.gdsc.smlm.results.filter.Filter) filter.getFilter()).toXML()
                        : fitConfig.getSmartFilterString()).append("\n");
                sb.append(((uk.ac.sussex.gdsc.smlm.results.filter.Filter) filter.getMinimalFilter()).toXML())
                        .append("\n");
            ...
            if (logger != null)
                LoggerUtils.log(logger, Level.INFO, "Slice %d: %d / %d = %s", slice, success, candidates.getSize(),
    ...
    private CandidateList indentifySpots(FitJob job, int width, int height, FitParameters params)
    {
        ...
        if (logger != null)
            LoggerUtils.log(logger, Level.INFO, "%d: Slice %d: %d candidates", workerId, slice, maxCandidate);
            ...
            float noise, double locationVariance)
    {
        ...
        if (inside)
            ...
            LoggerUtils.log(logger, Level.INFO, "[%d] Ignoring peak within image border @ %.2f,%.2f", slice,
            ...
            float meanIntensity, float[] params, float[] paramDevs, int id, double precision)
    {
        ...
        if (precision > 0)
        {
            final AttributePeakResult r = new AttributePeakResult(slice, origX, origY, origValue, error, noise,
                    meanIntensity, params, paramDevs);
            r.setId(id);
    ...
    private class CandidateSpotFitter
    {
        ...
        public CandidateSpotFitter(Gaussian2DFitter gf, ResultFactory resultFactory, double[] region, double[] region2,
                double[] var_g2, Rectangle regionBounds, int candidateId, FloatAreaSum area)
        {
            this.gf = gf;
        ...
        private double[] getPrecomputedFittedNeighbours()
        {
            if (precomputedFittedNeighboursMulti == null && precomputedFittedNeighbourCount != 0)
            {
                ...
                //uk.ac.sussex.gdsc.core.ij.Utils.display("precomputedFunctionValues", precomputedFittedNeighboursMulti, width, height);
            }
        ...
        public MultiPathFitResult.FitResult getResultMulti()
        {
            ...
            // Multiple-fit ...
            if (logger != null)
                LoggerUtils.log(logger, Level.INFO, "Slice %d: Multiple-fit (%d peaks : neighbours [%d + %d - %d])", slice, npeaks,
            ...
            //					uk.ac.sussex.gdsc.core.ij.Utils.display("Filtered", new FloatProcessor(dataBounds.width, dataBounds.height, filteredData));
            //			}
                ...
                double[] precomputedFunction)
        {
            ...
            final FastGaussianOverlapAnalysis overlap = new FastGaussianOverlapAnalysis(flags, null, spotParams, maxx,
                    maxy);
        ...
        private double[] getLocalStatistics(int n, double[] params)
        {
            ...
            result[0] = (stats[FloatAreaSum.INDEX_SUM] - f.integral(spotParams)) / stats[FloatAreaSum.INDEX_COUNT];
            if (result[0] < 0)
        ...
        public MultiPathFitResult.FitResult getResultDoubletMulti(double residualsThreshold)
        {
            ...
            //				if (!uk.ac.sussex.gdsc.core.utils.DoubleEquality.almostEqualRelativeOrAbsolute(residuals[i], residuals2[i], 1e-5,
            //						1e-6))
            //				{
            ...
            if (resultDoubletMulti != null && resultDoubletMulti.status == 0 && resultDoubletMulti.results != null)
            {
                ...
                final FitResult doubletFitResult = (uk.ac.sussex.gdsc.smlm.fitting.FitResult) resultDoubletMulti
                        .getData();
                ...
                final double[] initialParams = new double[initialParams2.length +
        ...
        public MultiPathFitResult.FitResult getResultSingle()
        {
            ...
            if (fittedNeighbourCount != 0)
            {
                ...
                final double[] precomputedFunctionParamsSingle = new double[1 +
                        parametersPerPeak * fittedNeighbourCount];
        ...
        private MultiPathFitResult.FitResult createResult(FitResult fitResult, PreprocessedPeakResult[] results,
                FitStatus fitStatus)
        {
            final MultiPathFitResult.FitResult mfitResult = new MultiPathFitResult.FitResult(fitStatus.ordinal(),
                    fitResult);
        ...
        public MultiPathFitResult.FitResult getResultDoubletSingle(double residualsThreshold)
        {
                    ...
                    functionParamsSingle != null)
            {
                // Recompute the deviations with all the parameters.
                // For equivalence with the multi-fit we only include the fits within the region.
                FitResult doubletFitResult = (uk.ac.sussex.gdsc.smlm.fitting.FitResult) resultDoubletSingle.getData();
        ...
        private QuadrantAnalysis computeQA(FitResult fitResult, Rectangle regionBounds, final double[] residuals)
        {
            ...
            if (logger != null)
                LoggerUtils.log(logger, Level.INFO, "Residue analysis = %f (%d,%d)", qa.score, qa.vector[0], qa.vector[1]);
                ...
                CandidateList peakNeighbours, QuadrantAnalysis qa, double singleValue, double localBackground)
        {
            ...
            if (newFitResult.getStatus() == FitStatus.OK)
            {
                ...
                )
                {
                    ...
                    improvement = ic2 < ic1;
                    if (logger != null)
                        LoggerUtils.log(logger, Level.INFO, "Model improvement - Log likelihood (IC) : %f (%f) => %f (%f) : %f", singleValue,
                ...
                else if (solverType == FunctionSolverType.WLSE)
                {
                    ...
                    improvement = ic2 < ic1;
                    if (logger != null)
                        LoggerUtils.log(logger, Level.INFO, "Model improvement - Chi-squared (IC) : %f (%f) => %f (%f) : %f", singleValue, ic1,
                ...
                else if (solverType == FunctionSolverType.LSE)
                {
                    ...
                    if (logger != null)
                        LoggerUtils.log(logger, Level.INFO, "Model improvement - Adjusted R^2 : %f => %f", singleValue, doubleValue);
                }
                else
                ...
                if (debugLogger != null)
                {
                    ...
                    LoggerUtils.log(debugLogger, Level.INFO, "Doublet %d [%d,%d] %s (%s) %s [%f -> %f] IC [%f -> %f] = %s\n",
                            slice, cc.fromRegionToGlobalX(cx), cc.fromRegionToGlobalY(cy), newFitResult.getStatus(),
                ...
                NEXT_PEAK: for (int n = 0; n < 2; n++)
                {
                    ...
                    {
                        if (logger != null)
                            LoggerUtils.log(logger, Level.INFO, "Fitted coordinates too far outside the fitted region (x %g || y %g) in %dx%d",
            ...
            if (logger != null)
                LoggerUtils.log(logger, Level.INFO, "Unable to fit 2-kernel model : %s", newFitResult.getStatus());
    ...
    private static boolean canIgnore(int x, int y, int xmin, int xmax, int ymin, int ymax, float height,
     ...
     */
    @SuppressWarnings("unused")
    private static int identifyRefitCandidates(int[] failed, int failedCount, float background, float noise,
    ...
    public void add(SelectedResult selectedResult)
    {
        ...
        for (int i = 0; i < results.length; i++)
        {
            ...
            if (peak.isNewResult())
            {
                ...
                if (logger != null)
                    ...
                    LoggerUtils.log(logger, Level.INFO, "Fit OK %d (%.1f,%.1f) [%d]: Shift = %.3f,%.3f : SNR = %.2f : Width = %.2f,%.2f",
        ...
        if (debugLogger != null)
        {
            ...
            LoggerUtils.log(debugLogger, Level.INFO, "%d:%d [%d,%d] %s (%s) = %s", slice, candidateId, cc.fromDataToGlobalX(x),
