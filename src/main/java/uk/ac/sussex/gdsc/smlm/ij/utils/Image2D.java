...
public abstract class Image2D
{
	 ...
	 */
	public static int checkSize(int nc, int nr, boolean raiseException) throws IllegalArgumentException
	{
		if (nc < 0 || nr < 0)
		{
			if (raiseException)
	...
	/**
	 * Instantiates a new 2D image. It is assumed that the sub-class will correctly create the data storage.
	 *
	 * @param nc
	 *            the number of columns
	 * @param nr
	 ...
	 *            the index
	 * @param xy
	 *            the xy components (must be an array of at least length 2)
	 * @throws IllegalArgumentException
	 *             if the index is not within the data
	 */
	 ...
	 *             if the region is not within the data
	 */
	public ImageProcessor cropToProcessor(int x, int y, int w, int h) throws IllegalArgumentException
	{
		// Check the region range
		if (x < 0 || w < 1 || (long) x + w > nc || y < 0 || h < 1 || (long) y + h > nr)
			throw new IllegalArgumentException("Region not within the data");
	 ...
	 * <p>
	 * If any of w,d are negative then the corresponding x,y is updated and the w,d is inverted.
	 * The maximum bounds of the given dimensions are then computed by adding the
	 * w,d to the x,y. The bounds are then clipped to the image dimensions and the intersect returned.
	 *
	 * @param x
	 *            the x index
	 * @param y
