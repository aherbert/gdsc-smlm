...
public abstract class ResultFilter {
   ...
   * @param results The fitted peaks
   */
  public abstract void filter(FitResult fitResult, int maxIndex, PeakResult... results);

  /**
   * Pass in a starting coordinate to be filtered. Called when fitting was unsuccessful but the
   * starting point can still be filtered.
   *
   * @param fitResult The output from the fitting routine
   * @param maxIndex The source index that was fitted
   * @param x The x position of the source index
   * @param y The y position of the source index
   */
