...
public class PCPALMClusters implements PlugIn
{
	...
	public void run(String arg)
	{
		...
		float[] xValues = ImageJUtils.createHistogramAxis(hist[0]);
		float[] yValues = ImageJUtils.createHistogramValues(hist[1]);
		...
		if (noiseData != null)
		{
			if (subtractNoise(histogramData, noiseData))
			{
				...
				xValues = ImageJUtils.createHistogramAxis(hist[0]);
				yValues = ImageJUtils.createHistogramValues(hist[1]);
				...
				if (autoSave)
				{
					String newFilename = ImageJUtils.replaceExtension(histogramData.filename, ".noise.tsv");
	...
	private HistogramData doClustering()
	{
		...
		ClusteringEngine engine = new ClusteringEngine(1, clusteringAlgorithm, new ImageJTrackProgress());
		if (multiThread)
			engine.setThreadCount(Prefs.getThreads());
		engine.setTracker(new ImageJTrackProgress());
		IJ.showStatus("Clustering ...");
		List<Cluster> clusters = engine.findClusters(convertToPoint(molecules), distance);
		IJ.showStatus("");
		...
		{
			results.add(new ExtendedPeakResult((float) c.getX(), (float) c.getY(), sd, c.getSize(), ++id));
		}
		...
		float yMax = (int) Math.ceil(MathUtils.max(values));
		int nBins = (int) (yMax + 1);
		float[][] hist = ImageJUtils.calcHistogram(values, 0, yMax, nBins);
	...
	private double[] fitBinomial(HistogramData histogramData)
	{
		...
		BinomialFitter bf = new BinomialFitter(new ImageJLogger());
