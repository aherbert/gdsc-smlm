...
public class CreateData implements PlugIn, ItemListener, RandomGeneratorFactory
{
		...
		if (simpleMode || benchmarkMode || spotMode)
		{
			...
			while (id < settings.getParticles())
			{
				...
				for (int j = 0; j < n; j++)
				{
					...
					final LocalisationModel m = new LocalisationModel(id, t, xyz, intensity, LocalisationModel.CONTINUOUS);
					localisations.add(m);
	...
	private void reportAndSaveFittingLimits(SpatialDistribution dist)
	{
		...
		if (min == max)
		{
			...
			final UnivariateLikelihoodFisherInformationCalculator c = new UnivariateLikelihoodFisherInformationCalculator(f,
					fiFunction);
	...
	private float[] createBackground(RandomDataGenerator random)
	{
		...
		if (settings.getBackground() > 0)
		{
			...
			if (uniformBackground)
			{
				...
				final UniformIntegerDistribution d = new UniformIntegerDistribution(random.getRandomGenerator(), 0, upper);
				while (samples-- > 0)
					pixels2[d.sample()] += 1;
