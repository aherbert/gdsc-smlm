...
public class DPGaussianFilter extends BaseWeightedFilter {
  ...
  private static final void downscaleLine(final float[] pixels, final float[] cache,
    ...
    int index = pixel0 + pointInc * (unscaled0 - reduceBy * 3 / 2); // pointer in pixels array
    final int pLast = pixel0 + pointInc * (length - 1);
    for (int xout = -1; xout <= newLength; xout++) {
      double sum0 = 0;
      double sum1 = 0;
      double sum2 = 0;
      for (int x = 0; x < reduceBy; x++, index += pointInc) {
        final double v = pixels[index < pixel0 ? pixel0 : (index > pLast ? pLast : index)];
