...
class BinomialFitterTest {
  ...
  private static void fitBinomial(UniformRandomProvider rg, int n, double p, boolean zeroTruncated,
    ...
    logger.log(TestLevel.TEST_INFO, FormatSupplier.getSupplier("Fitting (n=%d, p=%f)", n, p));
    int fail = 0;
    for (int i = 0; i < TRIALS; i++) {
      ...
      logger.log(TestLevel.TEST_INFO,
          FormatSupplier.getSupplier("  Fitted (n=%d, p=%f)", fittedN, fittedP));
      try {
    ...
    Assertions.assertTrue(fail <= FAILURES,
        FormatSupplier.getSupplier("Too many failures (n=%d, p=%f): %d", n, p, fail));
  }
  ...
  private static void fitBinomialUsingLseOrMle(UniformRandomProvider rg, int n, double p,
    ...
    for (int i = 0; i < TRIALS; i++) {
      ...
      logger.log(TestLevel.TEST_INFO, FormatSupplier
    ...
    logger.log(TestLevel.TEST_INFO,
        FormatSupplier.getSupplier("  Closest LSE %d, MLE %d", c1, TRIALS - c1));
    if (fail > FAILURES) {
