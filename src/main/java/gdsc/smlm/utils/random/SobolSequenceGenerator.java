 ...
 */
package gdsc.smlm.utils.random;
 ...
 * <p>
 * A Sobol sequence is a low-discrepancy sequence with the property that for all values of N, its subsequence (x1, ...
 * xN) has a low discrepancy. It can be used to generate pseudo-random points in a space S, which are equi-distributed.
 * <p>
 * The implementation already comes with support for up to 1000 dimensions with direction numbers calculated from <a
 * href="http://web.maths.unsw.edu.au/~fkuo/sobol/">Stephen Joe and Frances Kuo</a>.
 * <p>
 ...
 * @since 3.3
 * 
 * @note This class has been directly copied from the Apache Commons Math 3.3 source. The project depends on version 3.2
 *       and upgrading prevents distribution through the Fiji package of ImageJ which ships with v3.2.
 */
public class SobolSequenceGenerator implements RandomVectorGenerator
{
	 ...
	 * <p>
	 * The expected format is identical to the files available from <a
	 * href="http://web.maths.unsw.edu.au/~fkuo/sobol/">Stephen Joe and Frances Kuo</a>. The first line will be ignored
	 * as it is assumed to contain only the column headers. The columns are:
	 * <ul>
	 ...
	 */
	public SobolSequenceGenerator(final int dimension, final InputStream is) throws NotStrictlyPositiveException,
			MathParseException, IOException
	{
