...
public class FFastLog extends FastLog {
  ...
  /**
   * Create a new logarithm calculation instance. This will hold the pre-calculated log values for
   * base E and a table size depending on a given mantissa quantization.
   *
   * @param n The number of bits to keep from the mantissa. Table storage = 2^(n+1) * 4 bytes, e.g.
   *        64Kb for n=13
   */
  ...
  /**
   * Create a new logarithm calculation instance. This will hold the pre-calculated log values for a
   * given base and a table size depending on a given mantissa quantization.
   *
   * @param base the logarithm base (e.g. 2 for log duals, 10 for decibels calculations, Math.E for
   *        natural log)
   * @param n The number of bits to keep from the mantissa. Table storage = 2^(n+1) * 4 bytes, e.g.
   *        64Kb for n=13
   */
  public FFastLog(double base, int n) {
    ...
    // assert Math.abs((log2 - fastLog2(value)) / log2) < 1e-6f : String.format("[%d] log2(%g) %g !=
    // %g %g", i,
    // value, log2, fastLog2(value), uk.ac.sussex.gdsc.core.utils.FloatEquality.relativeError(log2,
    // fastLog2(value)));
    // }
  }
