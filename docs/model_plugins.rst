...
Fits candidates in a benchmark localisation image as a single or double spot (doublet) and scores the results. This image can be created by the ``Create Data`` plugin. Note that an idealised benchmark image with evenly spaced singles or doublets can be created using the ``Create Data`` plugin using the grid distribution. Alternatively the ``Load Benchmark Data`` plugin can load a set of ground-truth localisations and the simulation settings and associate them with a simulated image.
...
.. list-table::
   ...
   * - minN
     - The minimum number of photons (per second) for a flourophore.

   * - maxN
     - The maximum number of photons (per second) for a flourophore.

   * - N
     - The average number of photons (per frame) for a flourophore.
...
.. list-table::
   ...
   * - ss1
     - The residual sum-of-squares when fitting as a single spot (least square estimation).

   * - ss2
     - The residual sum-of-squares when fitting as a doublet (least square estimation).

   * - ll1
     - The log-likelihood when fitting as a single spot (maximum likelihood estimation).

   * - ll2
     - The log-likelihood when fitting as a doublet (maximum likelihood estimation).
