...
public class BenchmarkSpotFit implements PlugIn, ItemListener {
  ...
  public abstract class SpotMatch {
     ...
     *
     * @param i The index for the spot candidate
     * @param d The distance to the spot
  ...
  public class FitMatch extends SpotMatch {
     ...
     */
    public FitMatch(MultiPathPoint point, double d, double z, double predictedSignal,
        double actualSignal) {
      super(point.i, d, z);
      this.point = point;
  ...
  public class FilterCandidates implements Cloneable {
     ...
     *
     * @param p the positives
     * @param n the negatives
            ...
            if (!actualAssignment[assignment.getTargetId()])
              if (!predictedAssignment[assignment.getPredictedId()]) {
                ...
                if (point.type != MultiPathPoint.SPOT) {
                  ...
                  match[matchCount++] =
                      new CandidateMatch(point.i, d, p3.peakResult.getZPosition());
              }
        }
      }
  ...
  private boolean showDialog() {
    ...
    gd.addMessage(String.format("Fit candidate spots in the benchmark image created by "
  ...
  private TIntObjectHashMap<FilterCandidates> subsetFilterResults(
      TIntObjectHashMap<FilterResult> filterResults, int fitting) {
  ...
  private void summariseResults(TIntObjectHashMap<FilterCandidates> filterCandidates, long runTime,
    ...
    new HistogramPlotBuilder(TITLE, depthStats, "Match Depth (nm)")
  ...
  private double[] showDoubleHistogram(StoredDataStatistics[][] stats, final int i,
    ...
    if (showFilterScoreHistograms) {
      ...
      PlotWindow plotWindow = histogramPlot.show(wo);
      if (plotWindow == null) {
