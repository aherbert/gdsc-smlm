...
import uk.ac.sussex.gdsc.smlm.data.config.CalibrationProtos.CameraCalibration;
...
public class CalibrationWriter extends CalibrationReader {
   ...
   *
   * @param calibration the calibration (may be null)
   ...
   *
   * @param nmPerPixel image pixel size in nanometers
   */
   ...
   *
   * @param exposureTime the new exposure time
   */
   ...
   *
   * @param readNoise the new read noise
   */
   ...
   *
   * @param distanceUnit the new distanceUnit
   */
   ...
   *
   * @param intensityUnit the new intensityUnit
   */
  public void setIntensityUnit(IntensityUnit intensityUnit) {
    if (intensityUnit == null) {
      ...
      getBuilder().getIntensityCalibrationBuilder()
          .setIntensityUnitValue(intensityUnit.getNumber());
    }
  }
   ...
   *
   * @param angleUnit The new angleUnit
   */
  ...
  public void setPrecisionMethod(PrecisionMethod precisionMethod) {
    if (precisionMethod == null) {
      ...
      getBuilder().getResultDataCalibrationBuilder()
          .setPrecisionMethodValue(precisionMethod.getNumber());
    }
  }

  /**
   * Clear global camera settings. This should be used to remove the global camera settings, e.g.
   * bias, read noise, count/photon, for example when a per-pixel camera model is used such as sCMOS
   * camera type.
   */
