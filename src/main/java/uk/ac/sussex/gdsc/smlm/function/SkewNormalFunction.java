...
public class SkewNormalFunction {
  ...
  public double getSkewness() {
    final double sigma = getSigma();
    return ((4.0 - Math.PI) / 2.0) * (MathUtils.pow3((sigma * Math.sqrt(2.0 / Math.PI)))
        / Math.pow(1 - 2 * sigma * sigma / Math.PI, 1.5));
  }
   ...
   * @return the value
   */
  public static double evaluate(double x, double amplitude, double location, double scale,
      double alpha) {
    // return (2 * amplitude / scale) * normal((x - location) / scale) * cumul(alpha * (x -
    // location) / scale);
