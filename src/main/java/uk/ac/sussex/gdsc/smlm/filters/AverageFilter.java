...
public class AverageFilter {
   ...
   */
  public void rollingBlockAverageInternal(float[] data, final int maxx, final int maxy,
      final int n) {
    // Note: Speed tests show that this method is only marginally faster than
    // rollingBlockAverageNxNInternal.
   ...
   * @param n The block size
   */
  public void rollingBlockAverageNxNInternal(float[] data, final int maxx, final int maxy,
      final int n) {
    ...
    for (int x = 2; x < maxx - 2; x++) {
      ...
      for (int y = 0; y <= maxy - 5; y++, index++) {
        final float sum = newData[index] + newData[index + 1] + newData[index + 2]
            + newData[index + 3] + newData[index + 4];
  ...
  public void stripedBlockAverage5x5Internal(float[] data, final int maxx, final int maxy,
    ...
    for (int x = 2; x < maxx - 2; x++) {
      ...
      int index2 = x + 2 * maxx;
      for (int y = 0; y <= maxy - 5; y++, index++) {
        data[index2] = (w1 * (newData[index] + newData[index + 4]) + newData[index + 1]
  ...
  public void stripedBlockAverage7x7Internal(float[] data, final int maxx, final int maxy) {
    ...
    for (int y = 0; y < maxy; y++) {
      ...
      for (int x = 0; x <= maxx - 7; x++, index++) {
        final float sum = data[index] + data[index + 1] + data[index + 2] + data[index + 3]
    ...
    for (int x = 3; x < maxx - 3; x++) {
      ...
      for (int y = 0; y <= maxy - 7; y++, index++) {
        final float sum = newData[index] + newData[index + 1] + newData[index + 2]
            + newData[index + 3] + newData[index + 4] + newData[index + 5] + newData[index + 6];
        data[index2] = sum * divisor;
        index2 += maxx;
      }
    }
  ...
  public void stripedBlockAverage7x7Internal(float[] data, final int maxx, final int maxy,
    ...
    for (int x = 3; x < maxx - 3; x++) {
      ...
      for (int y = 0; y <= maxy - 7; y++, index++) {
        data[index2] =
            (w1 * (newData[index] + newData[index + 6]) + newData[index + 1] + newData[index + 2]
  ...
  public void stripedBlockAverage5x5(float[] data, final int maxx, final int maxy) {
    ...
    for (int y = 0; y < height; y++) {
      ...
      for (int x = 0; x < width; x++) {
        ...
        // Store result in transpose
        outData[centreIndex] =
            (row[x] + row[x + 1] + row[x + 2] + row[x + 3] + row[x + 4]) * divisor;
        centreIndex += height;
      }
    }
  }
  ...
  public void stripedBlockAverage5x5(float[] data, final int maxx, final int maxy, final float w) {
    ...
    for (int y = 0; y < height; y++) {
      ...
      for (int x = 0; x < width; x++) {
        ...
        // Store result in transpose
        outData[centreIndex] =
            (w1 * (row[x] + row[x + 4]) + row[x + 1] + row[x + 2] + row[x + 3]) * divisor;
        centreIndex += height;
      }
    }
  }
