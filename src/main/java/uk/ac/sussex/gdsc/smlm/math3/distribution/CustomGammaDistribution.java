...
public class CustomGammaDistribution extends AbstractRealDistribution {
  ...
  /**
   * The uninitialised flag. Indicates that the shape factors have not been computed for the current
   * shape.
   */
  private boolean uninitialised;

  /**
   * The constant value of {@code shape + g + 0.5}, where {@code g} is the Lanczos constant
   * {@link Gamma#LANCZOS_G}.
   */
  private double shiftedShape;
  /**
   * The constant value of
   * {@code shape / scale * sqrt(e / (2 * pi * (shape + g + 0.5))) / L(shape)}, where
   * {@code L(shape)} is the Lanczos approximation returned by {@link Gamma#lanczos(double)}. This
   * prefactor is used in {@link #density(double)}, when no overflow occurs with the natural
   * calculation.
   */
  private double densityPrefactor1;
  /**
   * The constant value of {@code shape * sqrt(e / (2 * pi * (shape + g + 0.5))) / L(shape)}, where
   * {@code L(shape)} is the Lanczos approximation returned by {@link Gamma#lanczos(double)}. This
   * prefactor is used in {@link #density(double)}, when overflow occurs with the natural
   * calculation.
   */
  private double densityPrefactor2;
  /**
   * Lower bound on {@code y = x / scale} for the selection of the computation method in
   * {@link #density(double)}. For {@code y <= minY}, the natural calculation overflows.
   */
  private double minY;
  /**
   ...
   * @throws NotStrictlyPositiveException if {@code shape <= 0} or {@code scale <= 0}.
   */
  public CustomGammaDistribution(RandomGenerator rng, double shape, double scale)
      throws NotStrictlyPositiveException {
  ...
  public double density(double x) {
     ...
     *
     * where a is the shape parameter, and b the scale parameter. Substituting the Lanczos
     * approximation of Gamma(a) leads to the following expression of the density
     *
     * a e 1 y a - sqrt(------------------) ---- (-----------) exp(a - y + g), x 2 pi (a + g + 0.5)
     * L(a) a + g + 0.5
     *
     * where y = x / b. The above formula is the "natural" computation, which is implemented when no
     * overflow is likely to occur. If overflow occurs with the natural computation, the following
     * identity is used. It is based on the BOOST library
     * http://www.boost.org/doc/libs/1_35_0/libs/math/doc/sf_and_dist/html/math_toolkit/special/
     * sf_gamma/igamma.html Formula (15) needs adaptations, which are detailed below.
     *
     * y a (-----------) exp(a - y + g) a + g + 0.5 y - a - g - 0.5 y (g + 0.5) = exp(a
     * log1pm(---------------) - ----------- + g), a + g + 0.5 a + g + 0.5
     *
     * where log1pm(z) = log(1 + z) - z. Therefore, the value to be returned is
     *
     * a e 1 - sqrt(------------------) ---- x 2 pi (a + g + 0.5) L(a) y - a - g - 0.5 y (g + 0.5) *
   ...
   * href="http://mathworld.wolfram.com/Chi-SquaredDistribution.html"> Chi-Squared Distribution</a>,
   * equation (9).</li> <li>Casella, G., &amp; Berger, R. (1990). <i>Statistical Inference</i>.
   * Belmont, CA: Duxbury Press.</li> </ul>
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * For shape parameter {@code alpha} and scale parameter {@code beta}, the mean is
   * {@code alpha * beta}.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * For shape parameter {@code alpha} and scale parameter {@code beta}, the variance is
   * {@code alpha * beta^2}.
   *
   * @return {@inheritDoc}
   */
