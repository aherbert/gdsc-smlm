...
The fit configuration is the same as in the ``Peak Fit`` plugin. As with the ``Peak Fit`` plugin the current settings are loaded when the plugin is initialised. If no settings exist then a default set of settings will be created. Note that the settings include the options for the candidate spot filter. Although not used to directly identify candidates the settings are used by the fitting engine for example to smooth input data for PSF parameter estimates and set bounds on neighbour regions.

When the plugin runs all the settings will be saved to the existing settings.

Note that the ``Fit Maxima`` plugin does not support the preview mode available in the ``Peak Fit`` plugin. The plugin will ignore any ROI crop region since candidates are predefined. To fit a crop requires pre-cropping of the candidates using the :numref:`{name} <results_plugins:ROI Crop Results>` plugin.
