...
public class DiffusionRateTest implements PlugIn
{
    ...
    public void run(String arg)
    {
        ...
        if (r2D.getN() > 0)
        {
            ...
            final String msg = "2D Diffusion rate = " + Utils.rounded(D, 4) + " um^2 / sec (" +
            ...
            RandomGenerator[] random)
    {
        ...
        if (n != 0)
        {
            ...
            final float[] params = PeakResult.createParams(0, n, (float) xyz[0], (float) xyz[1], 0);
            final float noise = 0.1f;
            final PeakResult r = new ExtendedPeakResult(peak, (int) params[PeakResult.X], (int) params[PeakResult.Y], n,
                    0, noise, 0, params, null, peak, id);
            results.add(r);
            if (last != null)
            {
                sum += last.distance2(r);
