...
/**
 * Run different filtering methods on a set of benchmark fitting results
 * outputting performance statistics on the success of the filter. The fitting
 * results are generated by the BenchmarkSpotFit plugin. <p> Filtering is done
 * using e.g. SNR threshold, Precision thresholds, etc. The statistics reported
...
public class BenchmarkFilterAnalysis implements PlugIn, FitnessFunction, TrackProgress {
    ...
    private static String[] COLUMNS = {
            ...
            // Scores against the original localisations. Calculated using the number of
            // localisations
    ...
    static {
        ...
        // Use the precision as criteria to ensure a set confidence on results labelled
        // as true
        criteriaIndex = COLUMNS.length - 3;
        // Score against the original data so we can compare across filters and fit
        // solvers
    ...
    public void run(String arg) {
        ...
        String timeString = ImageJUtils.timeToString(totalTime);
    ...
    /**
     * If filters have been provided in FiltersSets of 3 then expand the filters
     * into a set assuming the three represent min:max:increment.
     */
            ...
            if (filterSet.size() == 3 && filterSet.allSameType()) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (f1.getParameterValue(i) < f2.getParameterValue(i) && f3.getParameterValue(i) > 0
                        ...
                        increment[c][i] = inc;
                        long extra = 1;
                        // Check the current value is less than the max or (to avoid small round-off
                        // error)
                        // that the current value is closer to the max than the next value after the
                        // max.
        ...
        for (FilterSet filterSet : filterList) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (Filter f : list) {
                    ...
                    // We always have the min value set from the first filter so start at the next
                    // increment
                    for (double d = min + inc; d < max || d - max < max2 - d; d += inc) {
                        parameters[i] = d;
    ...
    private List<MemoryPeakResults> readResults() {
        if (resultsList == null || lastId != BenchmarkSpotFit.fitResultsId || lastRank != rerankBySignal
            ...
            // -=-=-=-
            // The scoring is designed to find the best fitter+filter combination for the
            // given spot candidates.
            // The ideal combination would correctly fit+pick all the candidate positions
            // that are close to a
            // localisation.
            //
            ...
            // Actual matches = Any spot candidate or fitted spot candidate that matches a
            // localisation
            // Fitted spots = Any spot candidate that was successfully fitted
            //
            // TP = A spot candidate that was fitted and matches a localisation and is
            // accepted
            // FP = A spot candidate that was fitted but does not match a localisation and
            // is accepted
            ...
            // TN = A spot candidate that failed to be fitted and does not match a
            // localisation
            // = A spot candidate that was fitted and does not match a localisation and is
            // rejected
            //
            // Since unfitted candidates can only be TN or FN we can accumulate these scores
            // and cache them.
            //
            // Using a distance ramped scoring function the degree of match can be varied
            // from 0 to 1.
            // Using a signal-factor ramped scoring function the degree of fitted can be
            // varied from 0 to 1.
            // When using ramped scoring functions the fractional allocation of scores using
            // the above scheme
            // is performed, i.e. candidates are treated as if they both match and unmatch.
            // This results in
            // an equivalent to multiple analysis using different thresholds and averaging
            // of the scores.
            //
            // The totals TP+FP+TN+FN must equal the number of spot candidates. This allows
            // different fitting
            ...
            // Recall = TP / (TP+FN) : This is valid between different fitting methods since
            // a method that
            // fits more spots will have a potentially lower FN
            // Jaccard = TP / (TP+FN+FP) : This is valid between fitting methods
            //
            // -=-=-=-
            // As an alternative scoring system, different fitting methods can be compared
            // using the same TP
            // value but calculating FN = localisations - TP and FP as Positives - TP. This
            // creates a score
            // against the original number of simulated molecules using everything that was
            // passed through the
            // filter (Positives). This score is comparable when a different spot candidate
            // filter has been used
            // and the total number of candidates is different, e.g. Mean filtering vs.
            // Gaussian filtering
            // -=-=-=-
            ...
            r.setConfiguration(XStreamXmlUtils.toXML(BenchmarkSpotFit.fitConfig));
            ...
            for (Entry<Integer, FilterCandidates> entry : BenchmarkSpotFit.fitResults.entrySet()) {
                ...
                int[] indices = SimpleArrayUtils.newArray(result.fitResult.length, 0, 1);
                if (rerankBySignal) {
                ...
                for (int i = 0; i < result.fitResult.length; i++) {
                    ...
                    if (fitResult.getStatus() == FitStatus.OK) {
                        ...
                        final float[] params = SimpleArrayUtils.toFloat(fitResult.getParameters());
                        // To allow the shift filter to function the X/Y position coordinates must be
                        // relative
                        ...
                        // Fuzzy classification uses a score of 1 (match) or 0 (no match), or >0 <1
                        // (partial match)
                ...
                // Accumulate the TN and FN scores for spot candidates that were not fitted but
                // match a result
                for (SpotMatch match : result.match) {
                    ...
                    matches += dScore;
                    c_fn += dScore;
                    // This should be '+= 1 - dScore' but we incremented c_tn above for all
                    // candidates that were not fitted
                    c_tn -= dScore;
                }
            }
    ...
    private boolean showDialog(List<MemoryPeakResults> resultsList) {
        ...
        if (best != null) {
            msg += String.format("\nCurrent Best=%s, FailCount=%d, Range=%d", MathUtils.rounded(best.score),
    ...
    /**
     * Run different filtering methods on a set of labelled peak results outputting
     * performance statistics on the success of the filter to an ImageJ table. <p>
     * If the peak result original value is set to 1 it is considered a true peak, 0
     * for a false peak. Filtering is done using e.g. SNR threshold, Precision
     * thresholds, etc. The statistics reported are shown in a table, e.g.
     * precision, Jaccard, F-score. <p> For each filter set a plot is shown of the
     * score verses the filter value, thus filters should be provided in ascending
    ...
    private void analyse(List<MemoryPeakResults> resultsList, List<FilterSet> filterSets) {
        ...
        if (showSummaryTable || saveTemplate) {
            ...
            for (FilterScore fs : filters) {
                ...
                // Show the recall at the specified depth. Sum the distance and signal factor of
                // all scored spots.
                ...
                sb.append('\t').append(MathUtils.rounded((double) tp / np)).append('\t')
                        .append(MathUtils.rounded(d / scored)).append('\t').append(MathUtils.rounded(sf / scored))
                        .append('\t');
                if (fs.atLimit)
                    sb.append('Y');
        ...
        if (!bestFilter.isEmpty()) {
            ...
            for (String type : bestFilterOrder) {
                ...
                for (int index = 0; index < parameters; index++) {
                    // For each parameter compute as upward + downward delta and get the average
                    // gradient
    ...
    private void addSensitivityScore(StringBuilder sb, double s, double s1, double s2, double dx1, double dx2) {
        // Use absolute in case this is not a local maximum. We are mainly interested in
        // how
    ...
    private int run(FilterSet filterSet, int setNumber, List<MemoryPeakResults> resultsList, final int count,
        ...
        if (evolve && allSameType) {
            ...
            for (int j = 0; j < indices.length; j++) {
                // Do not mutate parameters that were not expanded, i.e. the input did not vary
                // them.
            ...
            if (!gd.wasCanceled()) {
                ...
                // Override the settings with the step length, a min of zero and the configured
                // upper
                ...
                // If the initial population is huge ensure that the first selection culls to
                // the correct size
        ...
        // If the filters were expanded we can do a step search from the initial optimum
        // using the increment
        if (!evolve && stepSearch != 0 && allSameType && wasExpanded(setNumber)) {
            ...
            for (Filter filter : filterSet.getFilters()) {
                ...
                if (result[CRITERIA] >= minCriteria) {
                    ...
                    // If the same then check the criteria
                        ...
                        // If equal criteria then if the same type get the filter with the strongest
                        // params
            ...
            if (maxFilter != null) {
                ...
                if (!doSearch) {
                    ...
                    // Otherwise we will walk around the space we have already covered with a step
                    // size of 1.
        ...
        // Final evaluation does not need to assess all the filters if we have run
        // optimisation.
        ...
        // Note that count and total may have changed after running the genetic
        // algorithm
        ...
        // Update the final results window given that we mainly use
        // appendWidthoutUpdate(...) when adding data
        if (showResultsTable)
                ...
                for (int j = 0; j < indices.length; j++) {
                    ...
                    if (!wasNotExpanded(setNumber, p)) {
                        ...
                        if (c1 <= 0) {
                            ...
                            if (c1 == -1)
                                sb.append("<").append(MathUtils.rounded(lowerLimit[set][p]));
                            sb.append("]");
                        } else {
                            ...
                            if (c2 >= 0) {
                                sb.append(" : ").append(maxFilter.getParameterName(p)).append(" [")
                                        .append(MathUtils.rounded(value));
                                if (c2 == 1)
                                    sb.append(">").append(MathUtils.rounded(upperLimit[set][p]));
                                sb.append("]");
                            }
                        }
                    }
                ...
                if (sb.length() > 0) {
                    ...
                    if (criteriaPassed) {
                        ImageJUtils.log("Warning: Top filter (%s @ %s|%s) [%s] at the limit of the expanded range%s",
                                maxFilter.getName(), MathUtils.rounded((invertScore) ? -max[SCORE] : max[SCORE]),
        ...
        if (!criteriaPassed) {
            ImageJUtils.log("Warning: Filter does not pass the criteria: %s : Best = %s using %s", type,
                    MathUtils.rounded((invertCriteria) ? -max[CRITERIA] : max[CRITERIA]), maxFilter.getName());
            return 0;
        }

        boolean allowDuplicates = true; // This could be an option?

        // XXX - Commented out the requirement to be the same type to store for later
        // analysis.
        // This may break the code, however I think that all filter sets should be able
        // to have a best filter
    ...
    /**
     * When the two filters have equal scores, select the filter using the filter
     * parameters
     * 
     * @param filter1
     * @param filter2
    ...
    private double getScore(FractionClassificationResult s, final int index) {
        ...
        switch (index) {
        case 0:
            return s.getTruePositives() + s.getFalsePositives();
        case 1:
            return s.getTruePositives();
        case 2:
            return s.getFalsePositives();
        case 3:
            return s.getTrueNegatives();
        case 4:
            return s.getFalseNegatives();
        case 5:
            return s.getTruePositiveRate();
        case 6:
            return s.getTrueNegativeRate();
        case 7:
            return s.getPositivePredictiveValue();
        case 8:
            return s.getNegativePredictiveValue();
        case 9:
            return s.getFalsePositiveRate();
        case 10:
            return s.getFalseNegativeRate();
        case 11:
            return s.getFalseDiscoveryRate();
        case 12:
            return s.getAccuracy();
        case 13:
            return s.getMatthewsCorrelationCoefficient();
        case 14:
        ...
        case 19:
            return s.getJaccard();
        case 20:
            return getOriginalScore(s).getFalsePositives();
        case 21:
            return getOriginalScore(s).getFalseNegatives();
        case 22:
    ...
    private double[] run(Filter filter, List<MemoryPeakResults> resultsList, boolean subset, double tn, double fn,
        ...
        if (subset) {
            ...
            // FractionClassificationResult r2 = scoreFilter(filter,
            //// BenchmarkFilterAnalysis.resultsList);
            // if
            //// (!gdsc.smlm.utils.DoubleEquality.almostEqualRelativeOrAbsolute(r.getTruePositives(),
            //// r2.getTruePositives(), 1e-6, 1e-10) ||
            // !gdsc.smlm.utils.DoubleEquality.almostEqualRelativeOrAbsolute(r.getFalsePositives(),
            //// r2.getFalsePositives(), 1e-6, 1e-10) ||
            // !gdsc.smlm.utils.DoubleEquality.almostEqualRelativeOrAbsolute(r.getTrueNegatives(),
            //// r2.getTrueNegatives(), 1e-6, 1e-10) ||
            // !gdsc.smlm.utils.DoubleEquality.almostEqualRelativeOrAbsolute(r.getFalseNegatives(),
            //// r2.getFalseNegatives(), 1e-6, 1e-10))
            // {
            // System.out.printf("TP %f != %f, FP %f != %f, TN %f != %f, FN %f != %f\n",
            //// r.getTruePositives(), r2.getTruePositives(),
            // r.getFalsePositives(), r2.getFalsePositives(), r.getTrueNegatives(),
            //// r2.getTrueNegatives(), r.getFalseNegatives(), r2.getFalseNegatives());
            // }
        } else
            r = scoreFilter(filter, resultsList);

        // checkTotals(r.getTruePositives(), r.getFalsePositives(),
    ...
    private FractionClassificationResult fractionScoreSubset(Filter filter, List<MemoryPeakResults> resultsList,
        ...
        // This method uses a subset that was created using the fail count in origY so
        // no special method is necessary
    ...
    public StringBuilder createResult(Filter filter, FractionClassificationResult r) {
        ...
        addCount(sb, r.getTruePositives() + r.getFalsePositives(), i++);
        addCount(sb, r.getTruePositives(), i++);
        addCount(sb, r.getFalsePositives(), i++);
        addCount(sb, r.getTrueNegatives(), i++);
        addCount(sb, r.getFalseNegatives(), i++);

        add(sb, r.getTruePositiveRate(), i++);
        add(sb, r.getTrueNegativeRate(), i++);
        add(sb, r.getPositivePredictiveValue(), i++);
        add(sb, r.getNegativePredictiveValue(), i++);
        add(sb, r.getFalsePositiveRate(), i++);
        add(sb, r.getFalseNegativeRate(), i++);
        add(sb, r.getFalseDiscoveryRate(), i++);
        add(sb, r.getAccuracy(), i++);
        add(sb, r.getMatthewsCorrelationCoefficient(), i++);
        ...
        addCount(sb, m.getFalsePositives(), i++);
        addCount(sb, m.getFalseNegatives(), i++);
    ...
    private FractionClassificationResult getOriginalScore(FractionClassificationResult r) {
        ...
        // Note: We cannot calculate TN since this is the number of fit candidates that
        // are
        // filtered after fitting that do not match a spot or were not fitted.
        final double fp = r.getPositives() - r.getTruePositives();
        final double fn = simulationParameters.molecules - r.getTruePositives();
    ...
    private static void saveFilterSet(FilterSet filterSet, String filename) {
        ...
        try {
            ...
            out.write(XStreamXmlUtils.prettyPrintXml(XStreamWrapper.getInstance().toXML(list)));
        } catch (Exception e) {
    ...
    private MemoryPeakResults depthAnalysis(MemoryPeakResults results, Filter filter) {
        ...
        double[][] h1 = ImageJUtils.calcHistogram(depths, limits[0], limits[1], bins);
        double[][] h2 = ImageJUtils.calcHistogram(depthFitStats.getValues(), limits[0], limits[1], bins);
        ...
        double[][] h3 = ImageJUtils.calcHistogram(depths2, limits[0], limits[1], bins);
        ...
        PlotWindow pw1 = ImageJUtils.display(title1, plot1);
        if (ImageJUtils.isNewWindow())
            wo.add(pw1);

        // Interpolate
        final double halfBinWidth = (h1[0][1] - h1[0][0]) * 0.5;
        // Remove final value of the histogram as this is at the upper limit of the
        // range (i.e. count zero)
        ...
        // Perhaps allow configuration of the number of histogram bins and the smoothing
        // bandwidth.
        ...
        PlotWindow pw2 = ImageJUtils.display(title2, plot2);
        if (ImageJUtils.isNewWindow())
            wo.add(pw2);
    ...
    private void scoreAnalysis(MemoryPeakResults results, Filter filter) {
        ...
        final int bins = Math.max(10, simulationParameters.molecules / 100);
        double[][] h1 = ImageJUtils.calcHistogram(signal, limits1[0], limits1[1], bins);
        double[][] h2 = ImageJUtils.calcHistogram(distance, limits2[0], limits2[1], bins);
        ...
        // Build a histogram using the same limits
        double[][] h1b = ImageJUtils.calcHistogram(signal2, limits1[0], limits1[1], bins);
        double[][] h2b = ImageJUtils.calcHistogram(distance2, limits2[0], limits2[1], bins);

        // Draw distance histogram first
        String title2 = TITLE + " Distance Histogram";
        Plot2 plot2 = new Plot2(title2, "Distance (nm)", "Frequency");
        plot2.setLimits(limits2[0], limits2[1], 0, MathUtils.max(h2[1]));
        plot2.setColor(Color.black);
        ...
        plot1.setLimits(limits1[0], limits1[1], 0, MathUtils.max(h1[1]));
        plot1.setColor(Color.black);
    ...
    public class FilterScore implements Comparable<FilterScore> {
        ...
        public FilterScore(Filter filter, double score, double criteria, boolean atLimit) {
            update(filter, score, criteria, atLimit);
        }

        public void update(Filter filter, double score, double criteria, boolean atLimit) {
            this.filter = filter;
            this.score = score;
    ...
    /**
     * Initialise the results list used for scoring the filters. This is shared with
    ...
    public double fitness(Chromosome chromosome) {
        ...
        if (strictFitness) {
            ...
            // This will work unless we are scoring using the counts (i.e. scores that may
            // exceed 1)
            if (criteria >= minCriteria)
                score += 1;
        }
    ...
    public void shutdown() {
        ...
        // This filter may not have been part of the scored subset so use the entire
        // results set for reporting
    ...
    /**
     * Updates the given configuration using the latest settings used in
     * benchmarking spot filters, fitting and filtering.
     *
     * @param config    the configuration
     * @param useLatest Use the latest best filter. Otherwise use the highest
     *                  scoring.
    ...
    public static boolean updateConfiguration(FitEngineConfiguration config, boolean useLatest) {
        ...
        // We could set the fail count range dynamically using a window around the best
        // filter
    ...
    private static FilterResult getBestResult() {
        if (!scores.isEmpty()) {
            ...
            for (int i = 0; i < scores2.size(); i++) {
                ...
                double diff = MathUtils.round(score - max, 3);
                if (diff <= 0)
                    continue;
